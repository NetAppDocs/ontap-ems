...
[.lead]
ses.config events are presented when a stack of shelves contains more disk bays than the maximum number that are allowed which is typically fixed by following the documented corrective steps.
