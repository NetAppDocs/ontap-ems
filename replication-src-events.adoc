...
[.lead]
replication.src events are presented when the system detects an error on a SnapMirror(R) or SnapVault(R) source volume or qtree which typically does not require corrective action.
