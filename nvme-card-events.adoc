...
[.lead]
nvme.card events are presented when the NVMe caching module has gone into a degraded state due to a warning reported by the drive which may require reseating or replacing affected hardware.
