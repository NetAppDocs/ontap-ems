...
[.lead]
san.mccvserverinitfailed events are presented when, in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems, SAN Vserver failed to initialize during a switchover or switchback operation, which may require contacting NetApp support.
