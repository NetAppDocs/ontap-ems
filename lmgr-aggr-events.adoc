...
[.lead]
lmgr.aggr events are presented when locks established through CIFS Continuously Available (CA) shares on an aggregate are dropped after takeover, giveback, or aggregate relocation which is typically fixed by verifying system state and connectivity.
