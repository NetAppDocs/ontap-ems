...
[.lead]
raid.rg events are presented when a RAID group background long running I/O encounters an error that is fatal to the RAID group and the aggregate (cannot be fixed even by a mirror read from the other plex, if it exists) which may require contacting NetApp support.
