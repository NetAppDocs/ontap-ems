...
[.lead]
disk.fw events are presented when the system detects disks with firmware that is not up-to-date which is typically fixed by verifying system state and connectivity.
