...
[.lead]
vserver.vsdr events are presented when in Async Vserver-DR setup, when an auditing-configured Vserver is starting up in Read-Only (RO) mode which is typically fixed by following the documented corrective steps.
