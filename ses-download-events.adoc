...
[.lead]
ses.download events are presented when the power cycling channel event is issued after a disk shelf firmware download to disk shelves that require a power-cycle to activate the new code which typically does not require corrective action.
