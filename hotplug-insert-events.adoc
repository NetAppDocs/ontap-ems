...
[.lead]
hotplug.insert events are presented when the module type does not match the system's record for this slot which is typically fixed by following the documented corrective steps.
