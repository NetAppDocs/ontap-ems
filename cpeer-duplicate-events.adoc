...
[.lead]
cpeer.duplicate events are presented when an IP address from a peer cluster is also in use by the local cluster in the same IPspace which may require reseating or replacing affected hardware.
