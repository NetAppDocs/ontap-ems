...
[.lead]
reg.key events are presented when oNTAP(R) software cannot access the registry because the key is not valid which is typically fixed by following the documented corrective steps.
