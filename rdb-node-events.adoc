...
[.lead]
rdb.node events are presented when the replicated database (RDB) detects CPU starvation on this node, potentially causing it to leave cluster quorum which is typically fixed by following the documented corrective steps.
