...
[.lead]
config.badpoolassign events are presented when syncMirror(R) is in use and disks are assigned to different pools on the same interface or loop which may require reseating or replacing affected hardware.
