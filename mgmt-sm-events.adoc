...
[.lead]
mgmt.sm events are presented when a SnapMirror(R) protect operation fails because the source volume is already protected with a SnapMirror relationship in the specified destination Vserver with the same policy, schedule, relationship type, destination volume prefix, and destination volume suffix and auto-initialize parameter set to true which typically does not require corrective action.
...
== mgmt.sm.cg.child.dtch.rename
*Severity*::
NOTICE
*Description*::
This message occurs when SnapMirror geometry change job attempts to detach a child Consistency-group from a parent Consistency-group and there exists another Consistency-group with the same name as the child Consistency-group in the same SVM. SnapMirror job will rename the detaching child Consistency-group to a unique name so that there are no standalone Consistency-groups with the same name in an SVM.
*Corrective Action*::
(None).
*Syslog Message*::
Consistency-group "%s" has been detached from parent Consistency-group "%s" and renamed to "%s".
*Parameters*::
*cg_name* (STRING): Existing Name of Consistency-group before detach from parent consistency-group
*parent_cg_name* (STRING): Name of the Parent Consistency-group
*cg_renamed_to* (STRING): New Name of the Consistency-group
...
== mgmt.sm.create.crs.rel.absnt
*Severity*::
ERROR
*Description*::
This message occurs when the creation of a SnapMirror(R) relationship as part of configuration replication from a partner site fails in a MetroCluster(tm) configuration, potentially causing failures in switchback operations.
*Corrective Action*::
Delete the failed relationship from the partner site using the "snapmirror delete -destination-path <path>" command, and then recreate the relationship on this site.
*Syslog Message*::
SnapMirror relationship creation during configuration replication failed on destination "%s" due to empty relationship list received from partner site.
*Parameters*::
...
== mgmt.sm.rst.cleanup.timeout
*Severity*::
ALERT
*Description*::
This message occurs when a SnapMirror(R) restore relationship cleanup request times out, because the system was unable to determine whether the cleanup was successful. The DELETE API that deletes a SnapMirror restore relationship might return as failed even though the restore relationship was ultimately successfully deleted. The cleanup process might still be continuing in the background.
*Corrective Action*::
Use the "snapmirror show" command or the GET /api/snapmirror/relationships/ REST API to check whether the restore relationship has been deleted. If the restore relationship still exists, check the unhealthy_reason field in the response from the above GET API - this might indicate issues you can resolve before retrying the delete of the restore relationship. In "snapmirror show", the equivalent information is shown in the "last-transfer-error" field. If the problem persists, contact NetApp technical support.
*Syslog Message*::
Delete of restore relationship "%s" with source-path "%s" and destination-path "%s" took longer than expected.
*Parameters*::
*relID* (STRING): UUID of the restore relationship that timed out during cleanup.
*srcpath* (STRING): Source path of the restore relationship that timed out during cleanup.
*dstpath* (STRING): Destination path of the restore relationship that timed out during cleanup.
