...
[.lead]
cpeer.ip events are presented when an attempt is made to insert an IP address whose address family does not match the family style specified for the cluster peer relationship which is typically fixed by updating configuration.
