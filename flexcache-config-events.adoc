...
[.lead]
flexcache.config events are presented when the FlexCache volume expand operation fails to synchronize the FlexCache configuration information between the newly created constituent volume and the origin cluster which is typically fixed by updating configuration.
