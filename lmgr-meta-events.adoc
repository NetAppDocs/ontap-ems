...
[.lead]
lmgr.meta events are presented when this event occurs when the system detects that the lock manager host metafile is corrupted which is typically fixed by verifying system state and connectivity.
