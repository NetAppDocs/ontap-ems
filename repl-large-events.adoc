...
[.lead]
repl.large events are presented when the replication engine encounters a destination that cannot handle large 600TB volume limits which is typically fixed by updating configuration.
