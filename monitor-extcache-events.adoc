...
[.lead]
monitor.extcache events are presented when the monitor detects the WAFL(R) external cache subsystem (FlexScale) has failed and is no longer available for use which is typically fixed by following the documented corrective steps.
