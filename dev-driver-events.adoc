...
[.lead]
dev.driver events are presented when an I/O driver is throttling I/O requests because the underlying controller is overloaded with I/O requests which typically does not require corrective action.
