...
[.lead]
san.nosupportedsamenamedfcport events are presented when, in MetroCluster(tm) configurations on clustered Data ONTAP(R) systems, Data ONTAP cannot find the same named port that support the FCP/FC-NVMe data-protocol to place an FC logical interface (LIF) in the destination cluster, which is typically fixed by following the documented corrective steps.
