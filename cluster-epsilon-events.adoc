...
[.lead]
cluster.epsilon events are presented when automatic assignment of epsilon to a node fails, alerting the customer that there is no epsilon on their system which is typically fixed by updating configuration.
