...
[.lead]
ekmip.akv events are presented when a configured Azure Key Vault (AKV) that was previously reported unavailable for key operations is now available which typically does not require corrective action.
