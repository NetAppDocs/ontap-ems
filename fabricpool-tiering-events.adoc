...
[.lead]
fabricpool.tiering events are presented when oNTAP has stopped tiering data to the cloud tier because it has lost contact with the Service Control Unit used for one or more cloud subscription licenses which is typically fixed by following the documented corrective steps.
