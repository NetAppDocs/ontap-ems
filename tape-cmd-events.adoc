...
[.lead]
tape.cmd events are presented when a Check Condition status from the tape drive reports an informational condition or status to the controller which is typically fixed by verifying system state and connectivity.
