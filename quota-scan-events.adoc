...
[.lead]
quota.scan events are presented when the quota scanner aborts without finishing due to an error condition after the administrator uses the "volume quota on" command, which may require contacting NetApp support.
