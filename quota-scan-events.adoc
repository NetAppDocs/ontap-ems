...
[.lead]
quota.scan events are presented when after the administrator uses the "volume quota on" command, when the quota scanner aborts without finishing due to an error condition which may require contacting NetApp support.
