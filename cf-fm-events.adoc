...
[.lead]
cf.fm events are presented when at the start of a takeover, end of a successful takeover, start of root aggregate giveback, and completion of data aggregate giveback which typically does not require corrective action.
...
*Syslog Message*::
Failover monitor: giveback failed. Reason: %s.
*Parameters*::
...
(None).
*Syslog Message*::
Failover monitor: giveback started. "override-vetoes" set to %s.
*Parameters*::
...
== cf.fm.nwayPeerFwTransition
*Severity*::
NOTICE
*Description*::
This message occurs when there is a change in firmware state (i.e. failover state machine) of the HA peer node.
*Corrective Action*::
(None).
*Syslog Message*::
Firmware state of the HA peer node %s changed from %s to %s, progress counter: %u
*Parameters*::
*nodeuuid* (STRING): UUID of the HA peer node.
*prevstate* (STRING): Previously reported HA peer node's firmware state.
*newstate* (STRING): New firmware state, as reported by the HA peer node.
*progresscounter* (LONGINT): New progress counter, as reported by the HA peer node.
