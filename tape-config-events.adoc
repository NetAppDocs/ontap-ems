...
[.lead]
tape.config events are presented when the tape configuration file has a format error in a required parameter or is missing a required parameter which is typically fixed by following the documented corrective steps.
