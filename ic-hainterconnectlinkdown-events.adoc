...
[.lead]
ic.hainterconnectlinkdown events are presented when every hour when one of the high-availability (HA) interconnect links is down which is typically fixed by verifying system state and connectivity.
