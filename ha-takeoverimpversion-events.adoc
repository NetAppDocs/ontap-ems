...
[.lead]
ha.takeoverimpversion events are presented when the partner node is down; however, due to a version mismatch, it is currently impossible to perform a takeover which is typically fixed by following the documented corrective steps.
