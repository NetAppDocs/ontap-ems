...
[.lead]
arl.precmt events are presented when during aggregate relocation, when automatic lock resynchronization with the node's high-availability (HA) partner for files on this aggregate was aborted which typically does not require corrective action.
