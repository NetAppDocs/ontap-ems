...
[.lead]
arl.precmt events are presented when automatic lock resynchronization with the node's high-availability (HA) partner for files on this aggregate was aborted during aggregate relocation, which typically does not require corrective action.
