...
[.lead]
coredump.mcc events are presented when the coredump subsystem vetoes MetroCluster(tm) negotiated switchover because a coredump save is in progress which is typically fixed by verifying system state and connectivity.
