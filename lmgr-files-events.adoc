...
[.lead]
lmgr.files events are presented when a lock manager file object cannot be allocated due to reaching the limit for file objects in the associated pool which is typically fixed by verifying system state and connectivity.
