...
[.lead]
vifmgr.secondaryoffline events are presented when the system determines that a remote node has been offline for more than a minute and assigns logical network interfaces (LIFs) from that node to other available nodes in the cluster which typically does not require corrective action.
