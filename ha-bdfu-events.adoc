...
[.lead]
ha.bdfu events occur during storage failover (SFO) giveback or aggregate relocation when the source node cannot reenable background disk firmware update (BDFU) on the destination node.
