...
[.lead]
ha.bdfu events are presented when during storage failover (SFO) giveback or aggregate relocation when the source node cannot reenable background disk firmware update (BDFU) on the destination node which typically does not require corrective action.
