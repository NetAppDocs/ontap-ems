...
[.lead]
san.missinglicense events are presented when during replication of the FC/iSCSI service configuration, when the required service is not licensed or has expired in the cluster which is typically fixed by following the documented corrective steps.
