...
[.lead]
diskown.diskwasstolen events are presented when this event is issued when the ownership of a disk owned by the system is changed such that the disk is no longer owned by the system which typically does not require corrective action.
