...
[.lead]
callhome.aggr events are presented when after WAFL(R) Iron (wafliron) detects and repairs corruption in an aggregate metafile which is typically fixed by verifying system state and connectivity.
