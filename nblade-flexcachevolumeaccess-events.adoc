...
[.lead]
nblade.flexcachevolumeaccess events are presented when a client tries to access a FlexCache volume through an unsupported protocol which is typically fixed by following the documented corrective steps.
...
*Syslog Message*::
Attempt to access FlexCache volume with Master Data Set ID %u in SVM %s via %s denied for client %s.
*Parameters*::
*volMsid* (INT): Master Data Set ID of the FlexCache volume being accessed.
