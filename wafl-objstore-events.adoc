...
[.lead]
wafl.objstore EMS events occur when FabricPool aggregate and SnapMirror(R) tries to share the same underlying objectstore connection pools in a platform where the sharing is not supported.
