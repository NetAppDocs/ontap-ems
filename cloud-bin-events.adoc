...
[.lead]
cloud.bin events are presented when the cloud bin object copy operation (which copies objects from a source object store to a destination object store) fails because it is cannot read source objects due to unauthorized access to the object store container which is typically fixed by updating configuration.
