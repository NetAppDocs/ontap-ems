...
[.lead]
nblade.notactivevscanserver events are presented when a connection from a remote IP is not found in the Vscan scanner pool belonging to the storage virtual machine (SVM) which is typically fixed by following the documented corrective steps.
