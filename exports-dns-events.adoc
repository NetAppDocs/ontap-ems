...
[.lead]
exports.dns events are presented when a name services lookup request finds that DNS is configured as an ns-switch source for hosts, but DNS is not configured for the Vserver which is typically fixed by verifying system state and updating configuration.
