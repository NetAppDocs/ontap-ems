...
[.lead]
raid.read events are presented when the system detects a checksum block media error in an advanced zoned checksums(AZCS) RAID group during a read operation which typically does not require corrective action.
