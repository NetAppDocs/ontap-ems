...
[.lead]
monitor.partnercontroller events are presented when both controllers are configured as an HA (high-availability) pair, but the partner controller is not present which is typically fixed by following the documented corrective steps.
