...
[.lead]
san.fcnvmeunsupportedport events are presented when, in MetroCluster(tm) configurations on clustered Data ONTAP(R) systems, Data ONTAP has an FC NVMe (Nonvolatile Memory Express) logical interface (LIF) hosted on an unsupported FC port, which is typically fixed by verifying system state and updating configuration.
