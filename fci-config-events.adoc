...
[.lead]
fci.config events are presented when the system reboots automatically to enable correct adapter operation after changing the state of a reconfigurable adapter which is typically fixed by following the documented corrective steps.
