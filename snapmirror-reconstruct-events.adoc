...
[.lead]
snapmirror.reconstruct events are presented when the SnapMirror(R) requested reconstruction of a per-volume meta file used for labeling block information is complete which is typically fixed by following the documented corrective steps.
