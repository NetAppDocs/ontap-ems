...
[.lead]
raid.unsupported events are presented when configuration management detects that the disk belongs to a 32-bit aggregate, the use of which has been deprecated which may require reseating or replacing affected hardware.
