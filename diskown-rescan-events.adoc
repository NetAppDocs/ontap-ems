...
[.lead]
diskown.rescan events are presented when the high-availability (HA) messaging layer fails to connect to the specified node for disk ownership updates which is typically fixed by verifying system state and connectivity.
