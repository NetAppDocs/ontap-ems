...
[.lead]
nvmeof.mixedports events are presented when there is no longer a nonrecommended mix of a dedicated port and a nondedicated port cabled from a node to a shelf which typically does not require corrective action.
