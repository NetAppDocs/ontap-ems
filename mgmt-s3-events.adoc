...
[.lead]
mgmt.s3 events are presented when the total cluster-wide logical used size for S3 Snapmirror external object store relationships is near the license capacity limit but does not exceed the limit which is typically fixed by updating configuration.
