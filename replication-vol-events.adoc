...
[.lead]
replication.vol events are presented when snapMirror(R) or VolCopy replication encounters a disk error in the active map delta blocks on the destination and cannot continue replication which is typically fixed by updating configuration.
