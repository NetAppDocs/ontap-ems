...
[.lead]
spmgmt.driver events are presented when the system encounters an error while trying to get hourly statistics from the Service Processor (SP) which may require a reboot.
