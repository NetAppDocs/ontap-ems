...
[.lead]
clam.valid events are presented when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) determines that the local node is in a valid configuration for providing CLAM functionality which is typically fixed by following the documented corrective steps.
