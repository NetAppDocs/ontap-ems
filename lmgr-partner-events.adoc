...
[.lead]
lmgr.partner events are presented when a partner node's mirrored lock state goes out of sync with the actual lock state which is typically fixed by following the documented corrective steps.
