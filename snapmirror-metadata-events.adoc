...
[.lead]
snapmirror.metadata events occur when a Synchronous SnapMirror(R) destination flexible volume is unable to enter synchronous replication mode because the containing aggregate does not have enough space left to allow the update to proceed.
