...
[.lead]
snapmirror.metadata events are presented when a Synchronous SnapMirror(R) destination flexible volume is cannot enter synchronous replication mode because the containing aggregate does not have enough space left to allow the update to proceed which is typically fixed by following the documented corrective steps.
