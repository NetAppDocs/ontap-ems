...
[.lead]
acp.sock events are presented when the system encounters a socket error while setting up the Alternate Control Path (ACP) Management Ethernet port which is typically fixed by verifying system state and connectivity.
