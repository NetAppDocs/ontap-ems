...
[.lead]
wafl.fc events are presented when a FlexCache(R) origin node detects that its FlexCache volume has disconnected while a lock reclaim operation is in progress which is typically fixed by verifying system state and connectivity.
