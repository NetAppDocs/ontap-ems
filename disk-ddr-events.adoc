...
[.lead]
disk.ddr events are presented when the Dynamic Drive Recognition (DDR) Scanner detects a disk that is not part of its qualification table which is typically fixed by verifying system state and connectivity.
