...
[.lead]
pci.init events are presented when to warn about the time taken by PCI initialization when PCI configuration is waiting for outstanding background threads that might still be running which typically does not require corrective action.
