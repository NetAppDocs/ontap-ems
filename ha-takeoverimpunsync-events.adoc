...
[.lead]
ha.takeoverimpunsync events are presented when the partner node is down and it is currently impossible to perform a takeover because the takeover node does not have partner NVRAM data which is typically fixed by verifying system state and connectivity.
