...
[.lead]
secd.rpc events are presented when a user with the same domain and same client IP address is blocked due to continuous authentication requests with an invalid login password which is typically fixed by verifying system state and updating configuration.
