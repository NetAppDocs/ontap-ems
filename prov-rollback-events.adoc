...
[.lead]
prov.rollback events are presented when oNTAP(R) software cannot complete a rollback of a provisioning operation sequence that failed which is typically fixed by verifying system state and connectivity.
