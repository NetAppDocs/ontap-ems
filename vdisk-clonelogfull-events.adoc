...
[.lead]
vdisk.clonelogfull events are presented when lUN clone creation has failed because the limit on the maximum supported entries(32705) has been exceeded which is typically fixed by following the documented corrective steps.
