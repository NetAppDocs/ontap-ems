...
[.lead]
usb.device events are presented when multiple consecutive commands to the specified universal serial bus (USB) device are not completed within the allotted time which may require reseating or replacing affected hardware.
