...
[.lead]
nblade.junctionrootlookup2 events are presented when the system cannot obtain the root file handle of the volume mounted on a junction path which is typically fixed by verifying system state and connectivity.
