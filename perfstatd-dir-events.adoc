...
[.lead]
perfstatd.dir events are presented when the perfstat daemon (perfstatd) startup fails due to improper permissions for the '/mroot/etc/log' directory which may require restarting affected services or nodes.
