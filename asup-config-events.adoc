...
[.lead]
asup.config events are presented when this event is issued when we are cannot parse /etc/asup_content.conf which is typically fixed by following the documented corrective steps.
