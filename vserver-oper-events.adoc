...
[.lead]
vserver.oper events are presented when the system detects a cluster reboot on a Disaster Recovery Source (MetroCluster(tm) configuration or Vserver DR) which is typically fixed by following the documented corrective steps.
