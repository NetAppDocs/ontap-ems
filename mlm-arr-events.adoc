...
[.lead]
mlm.arr events are presented when the current processing of an error event prevents the Asymmetric Logical Unit Access (ALUA) handler from queuing an affinity switch event which typically does not require corrective action.
