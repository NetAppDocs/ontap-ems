...
[.lead]
callhome.client events are presented when a remote client accesses the node using Manage ONTAP(R) and triggers an event using the ems-autosupport-log API call with "auto-support" field set to "true" which may require contacting NetApp support.
