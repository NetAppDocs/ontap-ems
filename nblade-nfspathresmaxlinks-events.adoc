...
[.lead]
nblade.nfspathresmaxlinks events are presented when while resolving an input path, when the server has reached the limit of maximum links it can follow and the corresponding RPC fails which is typically fixed by following the documented corrective steps.
