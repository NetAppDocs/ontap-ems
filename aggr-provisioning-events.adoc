...
[.lead]
aggr.provisioning events are presented when is issued when the aggregate provisioning recommendation generated for the cluster is found to be incomplete due to the presence of unassigned disks which is typically fixed by verifying system state and connectivity.
