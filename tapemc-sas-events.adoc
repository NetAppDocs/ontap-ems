...
[.lead]
tapemc.sas events occur when Data ONTAP(R) detects a SAS device that is connected to a port that is dedicated for SAS disk storage attachment, but is not a disk shelf or a bridge device.
