...
[.lead]
config.sas2domain events are presented when a disk is multipathed to one SAS domain; that is, both ports of the disk are visible from the same HBA port which is typically fixed by following the documented corrective steps.
