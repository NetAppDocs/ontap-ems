...
[.lead]
dfu.suspended_disasterrecovery events are presented when, in a MetroCluster(TM) environment, firmware download to remote disks is suspended during disaster recovery, which is typically fixed by updating configuration.
