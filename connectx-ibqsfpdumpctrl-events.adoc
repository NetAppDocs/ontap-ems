...
[.lead]
connectx.ibqsfpdumpctrl events are presented when infiniBand retimer programming fails due to a quad small form-factor pluggable (QSFP) transceiver register dump failure, causing the control registers of QSFP to be dumped which is typically fixed by following the documented corrective steps.
