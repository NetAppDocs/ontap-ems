...
[.lead]
sfo.sendhome events are presented when the giveback request fails because there are no disks that belong to the partner which is typically fixed by verifying system state and updating configuration.
