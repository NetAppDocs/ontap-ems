...
[.lead]
acp.need events are presented when the Alternate Control Path (ACP) Administrator determines that the ACP processor is running an older firmware revision that supports only the Secure Socket Library (SSL) channel which is typically fixed by following the documented corrective steps.
