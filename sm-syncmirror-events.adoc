...
[.lead]
sm.syncmirror events are presented when a sync granular CG relationship transitions from in-sync status to out-of-sync status which is typically fixed by verifying system state and connectivity.
