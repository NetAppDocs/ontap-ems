...
[.lead]
wafl.snap events are presented when the system detects a possible change in the autodelete or autosize policy of the volume which is typically fixed by following the documented corrective steps.
...
ERROR
*Description*::
This message occurs when the creation of a Snapshot copy is skipped.
*Corrective Action*::
...
*Syslog Message*::
%s %s %s: Skipping creation of Snapshot %s (%s).
*Parameters*::
*vol* (STRING): Volume name.
*app* (STRING): Application UUID.
*volident* (STRING): The SVM's UUID, if the volume belongs to an SVM. Empty otherwise.
