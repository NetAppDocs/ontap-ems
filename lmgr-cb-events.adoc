...
[.lead]
lmgr.cb EMS events occur when a lock manager waiting lock callback object cannot be allocated due to reaching the limit for waiting lock callback objects in the associated pool.
