...
[.lead]
lmgr.cb events are presented when a lock manager waiting lock callback object cannot be allocated due to reaching the limit for waiting lock callback objects in the associated pool which is typically fixed by verifying system state and connectivity.
