...
[.lead]
scsi.path events are presented when excessive errors are encountered on this device within a short-enough time period to raise concern that there might be a faulty component between the storage initiator and device which is typically fixed by verifying system state and connectivity.
