...
[.lead]
sms.giveback events are presented when during an SFO aggregate giveback, when the SnapMirror(R) subsystem indicates that giveback should be vetoed but the veto was overridden by the automated nondisruptive update procedure which typically does not require corrective action.
