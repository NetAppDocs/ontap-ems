...
[.lead]
sas.adapter events are presented when the base WWN associated with the SAS adapter is of invalid format which is typically fixed by following the documented corrective steps.
...
ERROR
*Description*::
This message occurs when the WWN, a unique world wide name that identifies the adapter, fails to update.
*Corrective Action*::
