...
[.lead]
diff.svcs events are presented when the system cannot automatically apply the Differentiated Services (diff-svcs) setting to the High Availability (HA) partner which is typically fixed by updating configuration.
