...
[.lead]
recover.abort events are presented when this event is generated when restore has been aborted and currently ROOLR (Read Only On Line Replica) is running on the Filer which typically does not require corrective action.
