...
[.lead]
acp.locked events are presented when the link status of a locked wrench Ethernet port (on the back of the storage controller) changes from "up" to "down" which is typically fixed by verifying system state and connectivity.
