...
[.lead]
vifmgr.rdma events are presented when a logical interface (LIF) is moved from a port that does not support the required remote direct memory access (RDMA) protocols of the LIF which typically does not require corrective action.
