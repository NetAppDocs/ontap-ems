...
[.lead]
callhome.flash events are presented when flash Management Module (FMM) detects that a percentage of a Flash Cache is offline that is above a warning threshold which is typically fixed by following the documented corrective steps.
