...
[.lead]
nvmens.smbc events are presented when an NVMe namespace is partially restored to a SnapMirror(R) active sync volume because the SnapMirror(R) active sync volume does not support NVMe namespace which may require contacting NetApp support.
