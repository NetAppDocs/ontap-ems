...
[.lead]
fci.fabric events are presented when a switch in the fabric attached to the Fibre Channel adapter exceeds the maximum number of ports allowed on a switch which is typically fixed by following the documented corrective steps.
