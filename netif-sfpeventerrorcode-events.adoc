...
[.lead]
netif.sfpeventerrorcode events are presented when the installed Small Form-factor Pluggable (SFP+ or QSFP+) module is either not supported or faulty which is typically fixed by verifying system state and connectivity.
