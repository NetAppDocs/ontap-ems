...
[.lead]
vldb.mccaggregate events are presented when during MetroCluster(tm) configuration setup, when the aggregate at the destination site is renamed to handle a name collision which is typically fixed by following the documented corrective steps.
