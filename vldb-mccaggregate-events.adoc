...
[.lead]
vldb.mccaggregate events are presented when the aggregate at the destination site is renamed to handle a name collision during MetroCluster(tm) configuration setup, which is typically fixed by following the documented corrective steps.
