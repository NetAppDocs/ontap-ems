...
[.lead]
vserver.rbac events are presented when a custom role associated with a Storage Virtual Machine (SVM) has an aliased command directory associated with it which is typically fixed by following the documented corrective steps.
