...
[.lead]
diskown.disknotsup events are presented when newly inserted disks are only supported with software disk ownership enabled, but it is not enabled which is typically fixed by following the documented corrective steps.
