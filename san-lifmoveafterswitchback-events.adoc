...
[.lead]
san.lifmoveafterswitchback events are presented when a LIF cannot be placed on one of the disaster recovery (DR) nodes because it is offline during MetroCluster(tm) switchback, which is typically fixed by updating configuration.
