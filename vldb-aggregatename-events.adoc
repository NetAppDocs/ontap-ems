...
[.lead]
vldb.aggregatename events are presented when an aggregate's name in the Volume Location Database (VLDB) does not match the aggregate's current name on the node which typically does not require corrective action.
