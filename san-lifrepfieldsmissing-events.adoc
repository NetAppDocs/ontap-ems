...
[.lead]
san.lifrepfieldsmissing events are presented when the system cannot replicate a SAN logical interface (LIF) in a MetroCluster(tm) configuration, because one or more attributes of the source LIF cannot be determined which is typically fixed by verifying system state and connectivity.
