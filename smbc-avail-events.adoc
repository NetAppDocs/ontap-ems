...
[.lead]
smbc.avail events are presented when the primary-bias state is set for the secondary endpoints of the SnapMirror(R) active sync relationships because both of the SnapMirror(R) active sync clusters lose connectivity to the ONTAP Mediator which is typically fixed by verifying system state and connectivity.
