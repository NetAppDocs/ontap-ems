...
[.lead]
replication.qtree events are presented when the replication destination engine receives blocks with disk errors from the source for the first time in a transfer and continues with replication which may require contacting NetApp support.
