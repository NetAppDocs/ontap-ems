...
[.lead]
replication.qtree events occur when the replication destination engine receives blocks with disk errors from the source for the first time in a transfer and continues with replication.
