...
[.lead]
vsa.diskobjectstore events are presented when the system detects that a disk object store configuration is not synchronized between both nodes in a shared-storage High-Availability (HA) pair which is typically fixed by following the documented corrective steps.
