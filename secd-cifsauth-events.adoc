...
[.lead]
secd.cifsauth events are presented when a CIFS authentication attempt fails for any reason other than an unknown user name or bad password which is typically fixed by following the documented corrective steps.
