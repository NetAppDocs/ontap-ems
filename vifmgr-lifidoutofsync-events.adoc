...
[.lead]
vifmgr.lifidoutofsync events are presented when a Logical Interface (LIF) is configured in the Logical Interface manager (VIFMgr) but is not visible in the user interface." which is typically fixed by following the documented corrective steps.
