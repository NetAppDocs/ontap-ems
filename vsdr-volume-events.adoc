...
[.lead]
vsdr.volume events are presented when the volume to be created on the destination cluster fails because of an internal identity conflict with another volume in the system which is typically fixed by following the documented corrective steps.
