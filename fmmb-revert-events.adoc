...
[.lead]
fmmb.revert events are presented when this event is issued when the HA Mailbox opaque recieves a write request for the opaque area and a revert_to command is in progress which typically does not require corrective action.
