...
[.lead]
lmgr.badstructure events are presented when this event is issued whenever the multi-protocol lock manager finds a bad file pointer and turns off the logging of lock manipulation to preserve trace information which typically does not require corrective action.
