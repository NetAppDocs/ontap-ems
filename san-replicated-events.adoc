...
[.lead]
san.replicated events are presented when an unreplicated LUN becomes mapped to a replicated initiator group which is typically fixed by updating configuration.
