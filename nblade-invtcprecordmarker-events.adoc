...
[.lead]
nblade.invtcprecordmarker events are presented when a TCP connection is reset due to an invalid TCP record marker in an Open Network Computing/Remote Procedure Call request which is typically fixed by following the documented corrective steps.
