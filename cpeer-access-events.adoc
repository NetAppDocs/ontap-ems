...
[.lead]
cpeer.access events are presented when an access request from a peer cluster is rejected, either because the request is made to a restricted Vserver or to a restricted table which is typically fixed by verifying system state and connectivity.
