...
[.lead]
ems.engine events are presented when the Event Management System (EMS) fails to send an event to a mail destination due to name resolution lookup failures for the mail server specified in EMS configuration options which is typically fixed by verifying system state and updating configuration.
