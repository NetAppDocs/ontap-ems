...
[.lead]
netgroup.files events are presented when a netgroup lookup request finds that files is specified as a ns-switch source, but a netgroup file cannot be found which is typically fixed by verifying system state and connectivity.
