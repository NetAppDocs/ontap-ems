...
[.lead]
vserver.mcc events are presented when, in a MetroCluster(TM) configuration, an attempt to create a Vserver on the partner cluster fails, which is typically fixed by verifying system state and connectivity.
