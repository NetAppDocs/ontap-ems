...
[.lead]
wafl.mcc events are presented when wAFL(R) does not find the MetroCluster configuration information on this node which is typically fixed by verifying system state and connectivity.
