...
[.lead]
sfu.nsmdownloadfailed EMS events occur when a disk shelf firmware update fails to download disk shelf firmware to a disk shelf successfully.
...
*Corrective Action*::
Go to the NetApp Support Site at https://mysupport.netapp.com/site/downloads/firmware/disk-shelf-firmware and try downloading the disk shelf firmware again by using the (advanced privilege) "storage shelf firmware update" command.
*Syslog Message*::
