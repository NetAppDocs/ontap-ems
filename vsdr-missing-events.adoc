...
[.lead]
vsdr.missing events are presented when the clone parent volume does not exist during the Vserver SnapMirror(R)/MetroCluster(tm) baseline replication on the secondary site which is typically fixed by verifying system state and connectivity.
