...
[.lead]
san.lifspernodelimit events are presented when, in MetroCluster(tm) configurations on clustered Data ONTAP(R) systems, Data ONTAP cannot replicate a SAN or NVMe logical interface (LIF) in the destination cluster due to the LIF limits per node per Vserver, which may require a reboot.
