...
[.lead]
sis.upgrade events are presented when single Instance Storage (SIS) metafiles cannot be created in the aggregate containing the volume, during a Data ONTAP(R) upgrade which is typically fixed by following the documented corrective steps.
