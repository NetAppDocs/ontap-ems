...
[.lead]
sis.upgrade events occur when Single Instance Storage (SIS) metafiles cannot be created in the aggregate containing the volume, during a Data ONTAP(R) upgrade.
