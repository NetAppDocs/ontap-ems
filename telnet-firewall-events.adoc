...
[.lead]
telnet.firewall events are presented when telnet is automatically removed from firewall policies during a revert operation from Data ONTAP(R) 9.0 to 8.3 which is typically fixed by following the documented corrective steps.
