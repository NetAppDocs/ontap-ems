...
[.lead]
misconfig.lif events are presented when the record for the incorrectly placed LIF is lost during MetroCluster(TM) switchover or switchback which is typically fixed by updating configuration.
