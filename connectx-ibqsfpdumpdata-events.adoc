...
[.lead]
connectx.ibqsfpdumpdata events are presented when infiniBand retimer programming fails due to a quad small form-factor pluggable (QSFP) transceiver parse error, and 128 bytes of QSFP data are dumped which is typically fixed by following the documented corrective steps.
