...
[.lead]
cpeer.psk events are presented when the system receives an authentication request for a remote cluster whose peer authentication has been revoked by the local cluster administrator which is typically fixed by following the documented corrective steps.
