...
[.lead]
cpeer.psk events occur when the system receives an authentication request for a remote cluster whose peer authentication has been revoked by the local cluster administrator.
