...
[.lead]
tcpip.duplicate events are presented when the local node detects that it and some other device are using the same IP address which is typically fixed by verifying system state and updating configuration.
