...
[.lead]
vifmgr.clusterbd events are presented when oNTAP detects that the cluster broadcast domain is partitioned; some local ports in the cluster broadcast domain do not have layer-2 connectivity to one another which is typically fixed by verifying system state and connectivity.
