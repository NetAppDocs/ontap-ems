...
[.lead]
vsa.vsphere events are presented when data ONTAP(R) on VMware(R) manages to reconnect and authenticate itself with the vSphere(tm) server which is typically fixed by following the documented corrective steps.
