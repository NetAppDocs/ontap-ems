...
[.lead]
sas.unsupported events are presented when oNTAP detects that a cable is attached to an I/O module port that is permanently disabled and does not support attachment which is typically fixed by following the documented corrective steps.
