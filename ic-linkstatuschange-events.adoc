...
[.lead]
ic.linkstatuschange events are presented when the link status changes for a high-availability (HA) interconnect port which is typically fixed by verifying system state and connectivity.
