...
[.lead]
fg.aggr events are presented when the constituents of a FlexGroup volume are located on different types of aggregates (FabricPool-enabled aggregates and non-FabricPool-enabled aggregates) for more than one week which is typically fixed by updating configuration.
