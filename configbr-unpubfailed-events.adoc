...
[.lead]
configbr.unpubfailed events are presented when the configuration backup and restore feature fails to unpublish a package by using the file replication service which is typically fixed by verifying system state and connectivity.
