...
[.lead]
kern.cron events are presented when the kernel cron daemon is cannot reschedule an internal process due to an error in handling of daylight savings time, which typically does not require corrective action.
