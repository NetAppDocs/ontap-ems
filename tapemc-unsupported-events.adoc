...
[.lead]
tapemc.unsupported events are presented when a tape or media changer is detected that is not supported on this system, but its usage is not blocked which is typically fixed by following the documented corrective steps.
