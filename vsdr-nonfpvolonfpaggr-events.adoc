...
[.lead]
vsdr.nonfpvolonfpaggr events are presented when the source volume of an SVM disaster recovery (SVMDR) relationship residing on a non-FabricPool aggregate at the primary cluster is placed on a FabricPool aggregate at the secondary cluster which is typically fixed by following the documented corrective steps.
