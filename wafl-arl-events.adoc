...
[.lead]
wafl.arl events are presented when during the relocation of an aggregate, when the destination node fails the volume count limit and volume size limit checks which is typically fixed by following the documented corrective steps.
