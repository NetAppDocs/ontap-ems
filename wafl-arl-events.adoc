...
[.lead]
wafl.arl events are presented when the destination node fails the volume count limit and volume size limit checks during the relocation of an aggregate, which is typically fixed by following the documented corrective steps.
