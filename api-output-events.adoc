...
[.lead]
api.output events are presented when an invalid character was detected while checking the output for a Data ONTAP(R) API which is typically fixed by following the documented corrective steps.
