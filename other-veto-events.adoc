...
[.lead]
other.veto events are presented when a giveback or aggregate relocation operation fails but ONTAP(R) software cannot determine the specific operation type which is typically fixed by following the documented corrective steps.
