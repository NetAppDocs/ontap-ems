...
[.lead]
zsm.send events are presented when a socket timeout occurs on an intracluster Manage ONTAP(R) API request to an internal ONTAPI(R) server which is typically fixed by verifying system state and connectivity.
