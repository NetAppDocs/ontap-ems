...
[.lead]
exports.ngbh events are presented when a netgroup by host request fails because all ns-switch sources for the netgroup database have returned connection errors and files are unusable as a source which is typically fixed by verifying system state and connectivity.
