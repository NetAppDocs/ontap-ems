...
[.lead]
vsdr.app events are presented when during the configuration replication phase of a Vserver DR transfer when the consistency group that the system is trying to replicate already exists in the cluster on a different Vserver which is typically fixed by following the documented corrective steps.
