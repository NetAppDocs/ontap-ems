...
[.lead]
extcache.configunsupported events are presented when the controller contains an external cache hardware configuration that cannot be supported on the controller which is typically fixed by verifying system state and connectivity.
