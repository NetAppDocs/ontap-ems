...
[.lead]
am.dest events are presented when relocation of aggregates fails at the destination as we no longer accept aggregates from versions 8.2.0 or lower which is typically fixed by following the documented corrective steps.
