...
[.lead]
token.vol events are presented when a 'volume offline' operation results in clearing the token cache for that volume which typically does not require corrective action.
