...
[.lead]
sp.firmware events are presented when neither of the firmware images of the node's Service Processor (SP) or Baseboard Management Controller (BMC) on the primary flash chip can be booted which may require contacting NetApp support.
