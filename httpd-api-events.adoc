...
[.lead]
httpd.api events are presented when a Manage ONTAP(R) API request to an internal server expires due to configured session limits which is typically fixed by verifying system state and connectivity.
