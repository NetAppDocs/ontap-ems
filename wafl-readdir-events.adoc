...
[.lead]
wafl.readdir events are presented when a READDIR file operation has exceeded the timeout that it is allowed to run in WAFL which is typically fixed by verifying system state and connectivity.
