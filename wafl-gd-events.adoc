...
[.lead]
wafl.gd events are presented when reserving allocation size fails for a file using granular data because there is no space available in the volume which is typically fixed by updating configuration.
