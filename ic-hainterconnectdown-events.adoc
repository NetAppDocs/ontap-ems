...
[.lead]
ic.hainterconnectdown events are presented when the high-availability (HA) interconnect is down every hour, which is typically fixed by verifying system state and connectivity.
