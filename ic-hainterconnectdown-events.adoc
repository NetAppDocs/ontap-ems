...
[.lead]
ic.hainterconnectdown events are presented when every hour when the high-availability (HA) interconnect is down which is typically fixed by verifying system state and connectivity.
