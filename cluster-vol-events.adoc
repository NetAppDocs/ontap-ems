...
[.lead]
cluster.vol events are presented when the volume required to store the system data needed for cluster-wide storage is created successfully after one or more attempts to create it failed which typically does not require corrective action.
