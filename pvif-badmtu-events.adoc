...
[.lead]
pvif.badmtu events are presented when the interface group (ifgrp) driver fails to update the MTU size on a member port because it does not support the requested configuration which is typically fixed by following the documented corrective steps.
