...
[.lead]
tapemc.passthru events are presented when a data underrun (less data is transferred than requested) happens on a read operation to the device, and the number of data bytes reported as not transferred is unexpectedly larger than the number of bytes requested which is typically fixed by following the documented corrective steps.
