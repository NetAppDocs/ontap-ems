...
[.lead]
cache.ttl events are presented when on the last node during the upgrade to ONTAP(R) 9.3, if the default TTL values of any of the SecD or NFS export caches are changed which is typically fixed by updating configuration.
