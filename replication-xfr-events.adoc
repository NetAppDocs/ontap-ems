...
[.lead]
replication.xfr events occur when a SnapMirror(R)/SnapVault(tm) operation is not completed because a block received from the source system failed checksum verification.
