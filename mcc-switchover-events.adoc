...
[.lead]
mcc.switchover events are presented when a MetroCluster switchover operation finds a system metadata volume, used by config replication applications, in the nvfailed state which is typically fixed by verifying system state and connectivity.
