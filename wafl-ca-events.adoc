...
[.lead]
wafl.ca events are presented when the space used in the capacity tier of a FabricPool aggregate has exceeded or is projected to exceed the size limit set on the capacity tier which is typically fixed by updating configuration.
