...
[.lead]
wafl.write events are presented when a write to a file with CIFS-style space reservations fails due to a lack of space in the volume or aggregate which is typically fixed by updating configuration.
