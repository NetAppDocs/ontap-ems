...
[.lead]
vsdr.net events are presented when the system tries to automatically replicate a logical interface (LIF) in a MetroCluster(TM) configuration or Vserver DR setup and the IP address of the LIF that is being created already exists in another Vserver which is typically fixed by following the documented corrective steps.
...
== vsdr.net.LifPlacedOnDifferentIfgroup
*Severity*::
NOTICE
*Description*::
This message occurs when a logical interface (LIF) is placed on a different ifgroup than the source during replication. These ifgroups could be in different broadcast domains.
*Corrective Action*::
(None).
*Syslog Message*::
LIF %s in Vserver %s was placed on a different ifgroup (source: %s). Different ifgroups could be in different broadcast domains.
*Parameters*::
*lif* (STRING): LIF name.
*vserver_name* (STRING): Vserver name.
*source_ifgroup* (STRING): The ifgroup name of the LIF on the source cluster.

== vsdr.net.LifPlacedOnDifferentVlanTag
*Severity*::
NOTICE
*Description*::
This message occurs when a logical interface (LIF) is placed on a VLAN port with a different VLAN tag than the source during replication. Different VLAN tags may indicate different broadcast domains.
*Corrective Action*::
(None).
*Syslog Message*::
LIF %s in Vserver %s was placed on a port with a different VLAN tag (source tag: %s). Different VLAN tags may indicate different broadcast domains.
*Parameters*::
*lif* (STRING): LIF name.
*vserver_name* (STRING): Vserver name.
*source_vlan_tag* (STRING): The VLAN tag of the LIF on the source cluster.
