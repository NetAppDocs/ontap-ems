...
[.lead]
monitor.globalstatus events are presented when the system's global health is critical; for example, if a fan failed which is typically fixed by verifying system state and connectivity.
