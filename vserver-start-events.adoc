...
[.lead]
vserver.start events occur when a 'vserver start' operation fails due to failure to start either a logical interface (LIF) or a protocol for the Vserver specified.
