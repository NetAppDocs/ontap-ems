...
[.lead]
netif.eepromreadfailed events are presented when there is a failure in reading the electronically erasable programmable read-only memory (EEPROM) for a network interface which may require contacting NetApp support.
