...
[.lead]
flexcache.mcc events are presented when a FlexCache(R) configuration refresh operation fails during MetroCluster(tm) switchover and switchback which is typically fixed by verifying system state and connectivity.
