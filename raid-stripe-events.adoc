...
[.lead]
raid.stripe events are presented when we encounter a bad block buffer entry in NVRAM during stripe replay (indicating corrupted NVRAM), which may require reseating or replacing affected hardware.
