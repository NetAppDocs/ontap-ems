...
[.lead]
callhome.vfiler events are presented when on the destination controller during the final phase of migrating a vfiler when the 120 second time limit has been reached which is typically fixed by verifying system state and connectivity.
