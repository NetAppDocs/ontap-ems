...
[.lead]
adt.nfs events are presented when, in the auditing subsystem, the NFS root user is accessing the file without a valid CIFS credential, which is typically fixed by verifying system state and updating configuration.
