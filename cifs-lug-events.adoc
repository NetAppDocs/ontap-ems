...
[.lead]
cifs.lug events are presented when the job to import CIFS local users and groups is completed successfully only at a primary site within a MetroCluster(R) configuration or Vserver DR setup which is typically fixed by following the documented corrective steps.
