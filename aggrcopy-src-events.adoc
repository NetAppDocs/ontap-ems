...
[.lead]
aggrcopy.src events are presented when this event is issued when we are not able to obtain all the snapshot information, from the vol copy source volume, required for a vol copy transfer which typically does not require corrective action.
