...
[.lead]
callhome.shutdown events are presented when a disk drive fails but there are no suitable spares available for reconstruction which is typically fixed by updating configuration.
