...
[.lead]
backup.abort events are presented when this event is generated when backup event like dump has been aborted and currently ROOLR (Read Only On Line Replica) is running on the Filer which typically does not require corrective action.
