...
[.lead]
fci.nserr events are presented when the Fibre Channel fabric cannot successfully complete a Name Server query issued to obtain configuration and device information when an adapter is connected to the Fibre Channel fabric which may require contacting NetApp support.
