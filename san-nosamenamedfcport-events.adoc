...
[.lead]
san.nosamenamedfcport events are presented when, in MetroCluster(tm) configurations on clustered Data ONTAP(R) systems, Data ONTAP cannot find the same named port to place an FC/FC-NVMe logical interface (LIF) in the destination cluster, which is typically fixed by verifying system state and connectivity.
