...
[.lead]
vsdr.fpvolonnonfpaggr events are presented when a source volume of an SVM disaster recovery (SVMDR) relationship residing on a FabricPool aggregate at the primary cluster is placed on a non-FabricPool aggregate at the secondary cluster which is typically fixed by following the documented corrective steps.
