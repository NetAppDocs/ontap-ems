...
[.lead]
netif.badeeprom events are presented when the network interface has an electronically erasable programmable read-only memory (EEPROM) with a bad cyclic redundancy check (CRC) or media access control (MAC) address which may require reseating or replacing affected hardware.
