---
sidebar: sidebar
permalink: sms-clone-events.html
keywords: ems, message, messages, event, events, group, catalog
summary: sms.clone events
---

= sms.clone events
:toc: macro
:toclevels: 1
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
sms.clone events are presented when snapMirror(R) Synchronous fails a clone request because the relation is temporarily in a state that does not permit clone replication which typically does not require corrective action.


== sms.clone.request.disallow
*Severity*::
NOTICE
*Description*::
This message occurs when SnapMirror(R) Synchronous fails a clone request because the relation is temporarily in a state that does not permit clone replication.
*Corrective Action*::
(None).
*Syslog Message*::
Clone replication not permitted while in transitioning state.
*Parameters*::
(None).
