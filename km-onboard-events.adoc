...
[.lead]
km.onboard events are presented when a cluster administrator supplies the wrong Onboard Key Manager cluster-wide passphrase more than three times in a row while attempting to enable Common Criteria (CC) mode which typically does not require corrective action.
