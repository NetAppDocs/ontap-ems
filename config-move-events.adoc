...
[.lead]
config.move events are presented when a logical interface (LIF) migration fails in such a way that there might have been a resulting internal loss of the LIF IP address which is typically fixed by updating configuration.
