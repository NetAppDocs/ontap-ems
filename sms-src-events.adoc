...
[.lead]
sms.src events are presented when a SnapMirror(R) Synchronous operation has successfully cleared the persistent fence (read/write I/O has resumed) on the source volume which typically does not require corrective action.
