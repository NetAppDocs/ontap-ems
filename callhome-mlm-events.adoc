...
[.lead]
callhome.mlm events are presented when all paths for an array LUN are connected to the same controller on the array which is typically fixed by updating configuration.
