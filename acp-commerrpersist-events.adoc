...
[.lead]
acp.commerrpersist events are presented when the Alternate Control Path (ACP) Administrator determines that a SAS shelf I/O module (IOM) configured for in-band ACP is experiencing persistent communication errors which may require reseating or replacing affected hardware.
