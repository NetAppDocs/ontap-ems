...
[.lead]
raid.recons events are presented when during a RAID reconstruction operation, when the system detects an inconsistent RAID write signature on a RAID stripe which typically does not require corrective action.
