...
[.lead]
scsitarget.ispfct events occur when Fibre Channel host bus adapter(HBA) returns an event notification to the driver but the driver did not recognize the event type.
