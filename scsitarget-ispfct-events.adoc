...
[.lead]
scsitarget.ispfct events are presented when fibre Channel host bus adapter(HBA) returns an event notification to the driver but the driver did not recognize the event type which may require contacting NetApp support.
