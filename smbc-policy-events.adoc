...
[.lead]
smbc.policy events are presented when the system detects during a flip on a SnapMirror(R) Sync relationship that the custom policy has been deleted from the source cluster and automatically falls back to the system policy of type "automated-failover" which typically does not require corrective action.
