...
[.lead]
nblade.nfs4unoptimizedio events are presented when a session trunking group that is created by the NFSv41 client is split across multiple nodes which is typically fixed by verifying system state and connectivity.
