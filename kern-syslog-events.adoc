...
[.lead]
kern.syslog events are presented when this event is generated when we detect that syslog messages were dropped before being logged which is typically fixed by following the documented corrective steps.
