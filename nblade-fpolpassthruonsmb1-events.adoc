...
[.lead]
nblade.fpolpassthruonsmb1 events are presented when during a client attempt to establish an FPolicy pass-through read channel over SMB1, which is not supported which is typically fixed by verifying system state and connectivity.
