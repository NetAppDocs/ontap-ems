...
[.lead]
config.noadisk events are presented when the local node in an HA configuration is connected to its disk shelves through the shelf B module instead of the shelf A module which is typically fixed by following the documented corrective steps.
