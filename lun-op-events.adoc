...
[.lead]
lun.op events are presented when a LUN is restored or a LUN clone is created in a volume that is currently in the nvfailed state which is typically fixed by verifying system state and connectivity.
