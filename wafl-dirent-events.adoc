...
[.lead]
wafl.dirent events are presented when a user attempts to use a directory entry name that is not supported by the virtual server's language setting which is typically fixed by following the documented corrective steps.
