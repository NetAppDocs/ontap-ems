...
[.lead]
rpc.xprt events are presented when the number of active connections to a side band Remote Procedure Call (RPC) reaches the limit which is typically fixed by verifying system state and connectivity.
