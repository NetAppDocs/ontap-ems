...
[.lead]
openssh.unsupported events are presented when an unsupported cipher is used while attempting an SSH connection which is typically fixed by following the documented corrective steps.
