...
[.lead]
vsdr.volmove events are presented when a volume at the source of a Vserver-DR setup is encrypted, but the volume at the destination is not configured to be encrypted which is typically fixed by verifying system state and connectivity.
