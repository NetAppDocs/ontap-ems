...
[.lead]
cecc_log.masked events are presented when the ECC error logger stops reporting the occurrence of correctable ECC errors because the frequency of their occurrence is too high which is typically fixed by verifying system state and connectivity.
