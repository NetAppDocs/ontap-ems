...
[.lead]
rlm.network events are presented when the Remote LAN Module (RLM) detects a link error on the RLM network port which is typically fixed by verifying system state and connectivity.
