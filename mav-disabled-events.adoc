...
[.lead]
mav.disabled events are presented when multi-admin verification (MAV) is enabled, the bootarg "bootarg.multi_admin.enable" is set to false, and an operation that normally requires MAV approval is executed via the console which is typically fixed by updating configuration.
