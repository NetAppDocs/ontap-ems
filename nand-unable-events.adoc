...
[.lead]
nand.unable events are presented when data ONTAP(R) module that is responsible for monitoring the health of the NVRAM device cannot extract health information which is typically fixed by verifying system state and connectivity.
