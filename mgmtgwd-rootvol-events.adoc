...
[.lead]
mgmtgwd.rootvol events are presented when the system detects that a root volume might have changed, causing the local management databases to be out of sync with the replicated databases which may require contacting NetApp support.
