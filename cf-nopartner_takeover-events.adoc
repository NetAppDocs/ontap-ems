...
[.lead]
cf.nopartner_takeover events are presented when the partner node fails but requested not to be taken over to prevent multiple, simultaneous RAID reconstructs from starting which is typically fixed by verifying system state and connectivity.
