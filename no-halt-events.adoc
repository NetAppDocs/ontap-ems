...
[.lead]
no.halt events are presented when the option raid.timeout is set to 0 or if the option hasn't been set by the user, disabling automatic shutdown on a disk failure which is typically fixed by following the documented corrective steps.
