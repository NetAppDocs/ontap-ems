...
[.lead]
nblade.vifmove events are presented when there is a failure adding a Remote Direct Memory Access (RDMA) listener during a Logical Interface (LIF) move which is typically fixed by updating configuration.
