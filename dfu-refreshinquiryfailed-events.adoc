...
[.lead]
dfu.refreshinquiryfailed events are presented when after a successful disk firmware update, when the 'SCSI inquiry' command, which is issued to all disks as part of the disk firmware download process, fails to finish which is typically fixed by verifying system state and connectivity.
