...
[.lead]
largevol.remount events occur when the "is-large-size-enabled" property is set on a FlexVol volume or Flexgroup volume, which is NFS mounted for client access.
