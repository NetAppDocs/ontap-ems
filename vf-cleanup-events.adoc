...
[.lead]
vf.cleanup events are presented when some subsystems are cannot cleanup properly when a local host vfiler is destroyed or when a partner vfiler is being shutdown during giveback, which may require contacting NetApp support.
