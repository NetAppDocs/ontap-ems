...
[.lead]
vf.cleanup events occur when some subsystems are unable to cleanup properly when a local host vfiler is destroyed or when a partner vfiler is being shutdown during giveback.
