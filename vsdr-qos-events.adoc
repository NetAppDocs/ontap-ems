...
[.lead]
vsdr.qos events are presented when there are maximum number of QoS policy groups already existing on the destination cluster and the system attempts to create a new QoS policy group during QoS configuration replication in a MetroCluster(tm) setup, which is typically fixed by following the documented corrective steps.
