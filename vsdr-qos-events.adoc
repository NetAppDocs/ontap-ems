...
[.lead]
vsdr.qos events occur during QoS configuration replication in a MetroCluster(tm) setup when there are maximum number of QoS policy groups already existing on the destination cluster and the system attempts to create a new QoS policy group.
