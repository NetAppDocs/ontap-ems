...
[.lead]
ses.loop events are presented when the system finds a disk shelf with a Loop Resiliency Circuit (LRC) module before a disk shelf with an Electronically Switched Hub (ESH) module in the loop map which is typically fixed by following the documented corrective steps.
