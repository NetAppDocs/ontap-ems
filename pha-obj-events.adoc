...
[.lead]
pha.obj events are presented when oNTAP(R) detects that a blob has been deleted from a container which is typically fixed by following the documented corrective steps.
