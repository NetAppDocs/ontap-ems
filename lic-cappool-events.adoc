...
[.lead]
lic.cappool events occur when the system aborts an operation of reassigning nodes to a different capacity pool license, because another reassignment operation to a different capacity pool license in the same request has failed.
