...
[.lead]
disk.toomanynvmespindles events are presented when data ONTAP(R) finds more NVMe disks present on a platform than the platform actually supports which is typically fixed by updating configuration.
