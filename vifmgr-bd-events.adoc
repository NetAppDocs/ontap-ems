...
[.lead]
vifmgr.bd events are presented when an MTU is entered on a Broadcast Domain that its member ports cannot support which is typically fixed by following the documented corrective steps.
