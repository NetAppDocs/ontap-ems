...
[.lead]
cluster.ha events are presented when cluster HA has not been configured on a 2-node cluster with HA mode enabled which is typically fixed by updating configuration.
