...
[.lead]
csm.badconnection events are presented when oNTAP(R) software receives a Cluster Session Manager (CSM) connection with unrecognizable content which is typically fixed by verifying system state and updating configuration.
