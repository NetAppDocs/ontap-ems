...
[.lead]
auto.policy events occur during upgrade to Data ONTAP(R) 9.3 or later, when we start enabling the "auto" efficiency policy on the existing compatible volumes on All-Flash Optimized nodes in the background.
