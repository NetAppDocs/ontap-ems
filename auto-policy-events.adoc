...
[.lead]
auto.policy events are presented when we start enabling the "auto" efficiency policy on the existing compatible volumes on All-Flash Optimized nodes in the background during upgrade to Data ONTAP(R) 9.3 or later, which typically does not require corrective action.
