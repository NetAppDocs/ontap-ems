...
[.lead]
snmp.snmpv3 events are presented when sNMPv3 is disabled due to a cluster in FIPS mode being upgraded to a version of ONTAP(R) software supporting the SNMPv3 FIPS feature which is typically fixed by following the documented corrective steps.
