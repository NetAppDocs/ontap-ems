...
[.lead]
lmgr.move events are presented when a volume move operation fails the cutover phase, due to volume quiesce timer expiration, and not all locks were transferred before the failure which is typically fixed by verifying system state and connectivity.
