...
[.lead]
wafl.br events are presented when this event is issued if block reallocation has not been performed on an aggregate but a reallocated block was found that subsequently needs to be redirected which is typically fixed by following the documented corrective steps.
