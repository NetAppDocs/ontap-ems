...
[.lead]
clam.reenable events are presented when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) is reenabled to operate on this node after being intentionally disabled which may require a reboot.
