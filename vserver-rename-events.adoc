...
[.lead]
vserver.rename events are presented when the "vserver rename" command fails to update the peer Vserver name on the peer cluster in a Vserver peer relationship which is typically fixed by updating configuration.
