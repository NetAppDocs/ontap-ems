...
[.lead]
coredump.dr events are presented when coredump subsystem detects that disaster recovery (DR) partner sparecore dump did not start which is typically fixed by following the documented corrective steps.
