...
[.lead]
sfo.tkabort events are presented when the destination node cannot contact the object stores during the planned takeover of a FabricPool aggregate which is typically fixed by verifying system state and connectivity.
