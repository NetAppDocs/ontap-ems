...
[.lead]
config.mirroring events are presented when the name-service configuration is not replicated in the node which is typically fixed by verifying system state and connectivity.
