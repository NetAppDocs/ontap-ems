...
[.lead]
diskown.stealingdiskprevented events are presented when a "storage disk assign" operation is performed on a disk that is owned and has a reservation from another system which is typically fixed by following the documented corrective steps.
