...
[.lead]
lun.provisioning events are presented when during the creation of a LUN or NVME namespace when the provisioning for its metadata fails due to insufficient space in the aggregate which is typically fixed by following the documented corrective steps.
