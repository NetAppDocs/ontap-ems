...
[.lead]
raid.disktoc events are presented when this event is generated when it is detected from the disk Table Of Contents that a disk's stored rawsize does not match the physical size of the disk which typically does not require corrective action.
