...
[.lead]
raid.disktoc events occur when it is detected from the disk Table Of Contents that a disk's stored rawsize does not match the physical size of the disk.
