...
[.lead]
sms.status events occur when a SnapMirror(R) Sync initialize or resynchronize operation is completed successfully, and the source volume and destination volume are in "in-sync" status.
