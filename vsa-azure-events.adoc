...
[.lead]
vsa.azure events occur when ONTAP(R) Cloud on Microsoft(R) Azure(R) receives a probe, indicating that a transition to the expected normal state has occurred.
