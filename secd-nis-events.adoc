...
[.lead]
secd.nis events are presented when the server could not establish a TCP connection to a Network Information Service (NIS) server which is typically fixed by following the documented corrective steps.
