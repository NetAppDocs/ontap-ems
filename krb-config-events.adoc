...
[.lead]
krb.config events are presented when the CIFS server's security settings could not be written to the configuration file on this node which is typically fixed by verifying system state and updating configuration.
