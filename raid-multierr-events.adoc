...
[.lead]
raid.multierr events are presented when rAID encounters more errors than the RAID level protection affords, and RAID cannot recover the block which is typically fixed by verifying system state and connectivity.
