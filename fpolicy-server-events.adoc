...
[.lead]
fpolicy.server events are presented when the FPolicy server has applied backpressure which means that the server is busy and does not want to receive requests until it removes backpressure which typically does not require corrective action.
