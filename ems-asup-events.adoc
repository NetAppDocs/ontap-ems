...
[.lead]
ems.asup events are presented when the e-mail addresses for deprecated AutoSupport "noteto" messages are copied to EMS event notification destinations during an upgrade to Data ONTAP(R) 9.0 or later which is typically fixed by updating configuration.
