...
[.lead]
vserver.stop events are presented when a 'vserver stop' operation fails due to failure to stop either a logical interface (LIF) or a protocol for the Vserver specified which is typically fixed by following the documented corrective steps.
