...
[.lead]
vserver.stop events occur when a 'vserver stop' operation fails due to failure to stop either a logical interface (LIF) or a protocol for the Vserver specified.
