...
[.lead]
raid.cksum events are presented when this event is issued when WAFL accesses a block that was zeroed because it could not be recovered correctly which is typically fixed by following the documented corrective steps.
