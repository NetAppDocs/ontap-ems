...
[.lead]
replication.global events are presented when this event is issued when the request to change/set the throttling value for a replication association is failed which typically does not require corrective action.
