...
[.lead]
nse.op events are presented when oNTAP(R) does not succeed in performing an operation on a NetApp(R) Storage Encryption (NSE) self-encrypting drive (SED) which may require reseating or replacing affected hardware.
