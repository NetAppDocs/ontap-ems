...
[.lead]
ipsec.ob events are presented when the kernel cannot acquire an IPsec Security Association (SA) to protect an outbound packet which is typically fixed by verifying system state and connectivity.
