...
[.lead]
nblade.longrunningrpcop events are presented when a running remote procedure call (RPC) operation takes longer than a defined length of time which is typically fixed by verifying system state and connectivity.
