...
[.lead]
sfo.reassignfailed events are presented when during the Storage Failover takeover process, when the disks from the failed node cannot be reassigned due to the failure listed which is typically fixed by verifying system state and connectivity.
