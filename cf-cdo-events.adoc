...
[.lead]
cf.cdo events are presented when this event is issued when the filer receives a disk operation message over the High Availability (HA) interconnect that it does not recognize which is typically fixed by following the documented corrective steps.
