...
[.lead]
wafl.raid events are presented when during an aggregate transition workflow when an aggregate snapshot created in a 7-Mode environment is missing which is typically fixed by following the documented corrective steps.
