...
[.lead]
scsitarget.fcp events are presented when a small form-factor pluggable (SFP) is inserted into the Fibre Channel (FC) target adapter which is typically fixed by verifying system state and connectivity.
