...
[.lead]
config.bridge events are presented when an FC-to-SAS bridge is attached to an unsupported FC adapter port which is typically fixed by following the documented corrective steps.
