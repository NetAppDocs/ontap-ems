...
[.lead]
lifp.net events are presented when while assigning network ports to broadcast domains, when LIF Placement subnet discovery finds a port that is assigned to a broadcast domain that is different from the broadcast domain to which other ports in the same physical broadcast domain are assigned which is typically fixed by following the documented corrective steps.
