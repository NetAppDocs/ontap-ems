...
[.lead]
callhome.nvdimm events are presented when this event occurs when excessive correctable error-correcting code (ECC) memory errors are reported by the NVDIMM which is typically fixed by verifying system state and connectivity.
