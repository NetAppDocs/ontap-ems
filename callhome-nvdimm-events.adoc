...
[.lead]
callhome.nvdimm events are presented when excessive correctable error-correcting code (ECC) memory errors are reported by the NVDIMM, which is typically fixed by verifying system state and connectivity.
