...
[.lead]
lmgr.session events are presented when this event occurs when a SpinNP session has failed, which causes session-scoped (e.g which is typically fixed by following the documented corrective steps.
