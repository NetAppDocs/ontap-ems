...
[.lead]
upgrademgr.two events are presented when a two-stage version update starts on the cluster where the target release is two versions later than the current version which typically does not require corrective action.
