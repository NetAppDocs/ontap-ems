...
[.lead]
login.auth events are presented when this event is issued when a login attempt is refused due to authentication failure which is typically fixed by following the documented corrective steps.
