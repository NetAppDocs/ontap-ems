...
[.lead]
ipmultipath.session events are presented when an IP Multipath session is dropped due to failure to allocate memory which is typically fixed by following the documented corrective steps.
