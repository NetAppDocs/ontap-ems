...
[.lead]
cpeer.ping events are presented when an attempt is made to check the cluster peering relationship and no intercluster LIFs of the same address family and the correct IPspace are found on this node which is typically fixed by updating configuration.
