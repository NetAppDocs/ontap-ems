...
[.lead]
nblade.vscanconninvaliduser events are presented when during a client attempt to establish a vscan connection, when the logged-in user of that client is not found in the list of allowed users which is typically fixed by verifying system state and connectivity.
