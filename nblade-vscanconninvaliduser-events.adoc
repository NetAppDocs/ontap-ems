...
[.lead]
nblade.vscanconninvaliduser events are presented when the logged-in user of that client is not found in the list of allowed users during a client attempt to establish a vscan connection, which is typically fixed by verifying system state and connectivity.
