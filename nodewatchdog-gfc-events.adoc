...
[.lead]
nodewatchdog.gfc events occur when the storage system is low in networking buffers (mbuf) for longer than the duration threshold, which indicates abnormal circumstances.
