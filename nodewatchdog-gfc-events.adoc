...
[.lead]
nodewatchdog.gfc events are presented when the storage system is low in networking buffers (mbuf) for longer than the duration threshold, which indicates abnormal circumstances which may require a reboot.
