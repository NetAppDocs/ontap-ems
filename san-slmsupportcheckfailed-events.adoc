...
[.lead]
san.slmsupportcheckfailed events are presented when during reboot when SAN capability is enabled, the cluster has more than 8 nodes and lun.reporting.nodes options is disabled which is typically fixed by updating configuration.
