...
[.lead]
vfilermigrate.diskownershipmismatch events are presented when the SnapMover feature is licensed but the disk ownership model is not using the software-based disk ownership model which is typically fixed by following the documented corrective steps.
