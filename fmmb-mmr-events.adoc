...
[.lead]
fmmb.mmr events are presented when this event is issued when the outbound FM payload area is found to be corrupted and the master mailbox record is missing which typically does not require corrective action.
