...
[.lead]
scsi.security events are presented when a self-encrypting drive (SED) reports excessive sequential failed authentication attempts with an incorrect Authentication Key (AK) which is typically fixed by verifying system state and connectivity.
