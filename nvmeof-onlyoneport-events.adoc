...
[.lead]
nvmeof.onlyoneport events are presented when there is no longer only one port cabled from a node to a shelf which typically does not require corrective action.
