...
[.lead]
ctl.session events are presented when iSCSI CAM target layer session status is changed which is typically fixed by following the documented corrective steps.
