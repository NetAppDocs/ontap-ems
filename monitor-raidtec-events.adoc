...
[.lead]
monitor.raidtec events are presented when a RAID-TEC aggregate is in double degraded mode as part of an hourly system check, which is typically fixed by verifying system state and connectivity.
