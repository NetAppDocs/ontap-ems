...
[.lead]
monitor.raidtec events are presented when part of an hourly system check, when a RAID-TEC aggregate is in double degraded mode which is typically fixed by verifying system state and connectivity.
