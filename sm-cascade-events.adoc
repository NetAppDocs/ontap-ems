...
[.lead]
sm.cascade events are presented when concurrent SnapMirror(R) transfers are attempted while in a cascaded scenario, which is not supported which typically does not require corrective action.
