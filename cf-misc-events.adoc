...
[.lead]
cf.misc events are presented when the message occurs when a giveback is initiated before the partner has reached waiting-for-giveback state which typically does not require corrective action.
