...
[.lead]
boot.varfs events are presented when an error is detected while trying to back up the /var file system to the boot media or node root volume (mroot) which may require contacting NetApp support.
...
*Syslog Message*::
I/O error caused forced umount of the /var file system. %s
*Parameters*::
*err_str* (STRING): Error string.
