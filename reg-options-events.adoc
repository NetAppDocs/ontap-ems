...
[.lead]
reg.options events are presented when an option is changed on one cluster node but applies to both the local and partner when the cluster is in takeover mode which typically does not require corrective action.
