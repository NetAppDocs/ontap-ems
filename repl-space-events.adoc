...
[.lead]
repl.space events are presented when the attempt to change space guarantee type fails on the destination volume which is typically fixed by following the documented corrective steps.
