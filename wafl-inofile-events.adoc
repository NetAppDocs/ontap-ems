...
[.lead]
wafl.inofile events are presented when this event can occur when a snapshot's inofile buffer header has the wrong magic or file block number which is typically fixed by following the documented corrective steps.
