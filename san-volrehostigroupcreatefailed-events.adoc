...
[.lead]
san.volrehostigroupcreatefailed events are presented when Data ONTAP fails to create igroup on the destination Vserver on clustered Data ONTAP(R) systems, during Volume Rehost operation, which is typically fixed by following the documented corrective steps.
