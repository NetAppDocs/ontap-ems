...
[.lead]
san.volrehostigroupcreatefailed events are presented when on clustered Data ONTAP(R) systems, during Volume Rehost operation when Data ONTAP fails to create igroup on the destination Vserver which is typically fixed by following the documented corrective steps.
