...
[.lead]
aggrcopy.dst events are presented when this event is issued when an aggregate copy start is issued on an aggregate which is offline which is typically fixed by following the documented corrective steps.
