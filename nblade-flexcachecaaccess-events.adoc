...
[.lead]
nblade.flexcachecaaccess events are presented when a client tries to access a FlexCache(R) volume via SMB3 using a continuously available share but is denied because FlexCache does not support persistent handle capability which is typically fixed by following the documented corrective steps.
