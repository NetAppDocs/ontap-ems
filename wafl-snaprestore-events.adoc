...
[.lead]
wafl.snaprestore events are presented when during a snapshot restore operation, when the system cannot recognize the version of Data ONTAP(R) from the snapshot it is reverting to which is typically fixed by following the documented corrective steps.
