...
[.lead]
clam.node events are presented when the local Connectivity, Liveliness and Availability Monitor (CLAM) concludes that the local node is now disabled which is typically fixed by following the documented corrective steps.
