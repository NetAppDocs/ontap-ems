...
[.lead]
snaplock.vol events are presented when this event is generated when the SnapLock(R) volume compliance clock time is skewed from the SnapLock system compliance clock by more than one day which typically does not require corrective action.
