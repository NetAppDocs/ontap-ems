...
[.lead]
net.ifgrp events are presented when the system fails to create a new interface group because the number of such groups already present on the node exceed either the number of physical ports on the node or the upper bound on number of interface groups supported which is typically fixed by following the documented corrective steps.
