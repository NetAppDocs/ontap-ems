...
[.lead]
smc.restore events are presented when a SnapMirror restore operation or a SnapMirror restore cleanup operation fails to cleanup a SnapMirror restore source relationship information which is typically fixed by verifying system state and connectivity.
