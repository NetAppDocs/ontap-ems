...
[.lead]
api.fileio events are presented when after a file I/O error that typically causes a Data ONTAP(R) API to fail which is typically fixed by following the documented corrective steps.
