...
[.lead]
wafl.metadata events are presented when the space reserved for volume metadata is decreased when the value of "space-guarantee" for a FlexClone volume is changed from "volume" to "none" which typically does not require corrective action.
