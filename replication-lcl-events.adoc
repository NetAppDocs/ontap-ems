...
[.lead]
replication.lcl events are presented when a SnapMirror(R) or SnapVault(R) transfer is started on a system without a host name which is typically fixed by updating configuration.
