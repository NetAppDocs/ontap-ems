...
[.lead]
sp.dup events are presented when another system has been detected to be using the same IP address as the one configured on the Service Processor (SP) which is typically fixed by following the documented corrective steps.
