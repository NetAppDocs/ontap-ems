...
[.lead]
vldb.adtcons events are presented when the Remote Procedure Call (RPC) from the Volume Location Database (VLDB) to the Management Gateway (MGWD) fails which is typically fixed by updating configuration.
