...
[.lead]
sysmgr.upload events are presented when an attempt to upload a file through the System Manager web service fails because the HTTP method is not POST which is typically fixed by following the documented corrective steps.
