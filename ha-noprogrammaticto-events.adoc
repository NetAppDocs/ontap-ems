...
[.lead]
ha.noprogrammaticto events are presented when a takeover is initiated programmatically by Data ONTAP(R) and the takeover request fails because giveback or aggregate relocation cleanup is still in progress which typically does not require corrective action.
