...
[.lead]
scsiblade.illegal events are presented when an error is returned for a SCSI command that exceeds the LUN's maximum supported transfer length of 1MB which may require a reboot.
