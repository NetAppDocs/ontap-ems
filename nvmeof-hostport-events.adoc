...
[.lead]
nvmeof.hostport events are presented when a node shelf is no longer cabled incorrectly from a port that is not a storage port which typically does not require corrective action.
