...
[.lead]
sfu.downloaderror EMS events occur when a disk shelf firmware update fails to successfully download firmware to a disk shelf or shelves in the system.
...
*Corrective Action*::
Redownload the latest disk shelf firmware from the NetApp Support Site at https://mysupport.netapp.com/site/downloads/firmware/disk-shelf-firmware and attempt to download disk shelf firmware again, using the (advanced privilege) "storage shelf firmware update" command.
*Syslog Message*::
