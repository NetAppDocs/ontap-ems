...
[.lead]
block.giveback events occur during a storage failover (SFO) aggregate giveback, when a SnapMirror(R) active sync automatic unplanned failover operation is in progress.
