...
[.lead]
scsitarget.slifct events are presented when the Fibre Channel target device driver encounters an unrecognized asynchronous event notification from the firmware, possibly as a result of a firmware update which may require contacting NetApp support.
