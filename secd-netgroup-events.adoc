...
[.lead]
secd.netgroup events are presented when if the filter for searching the Lightweight Directory Access Protocol (LDAP) server is found to be invalid which is typically fixed by verifying system state and connectivity.
