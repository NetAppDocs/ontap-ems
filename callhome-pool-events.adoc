...
[.lead]
callhome.pool events are presented when an aggregate does not comply with the current Capacity Pool entitlement, as determined by the Licensing Manager which is typically fixed by verifying system state and connectivity.
