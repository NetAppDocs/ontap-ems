...
[.lead]
monitor.shelf events are presented when the monitor detects that a disk shelf access error has occurred which is typically fixed by verifying system state and connectivity.
