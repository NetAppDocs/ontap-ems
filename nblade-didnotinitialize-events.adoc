...
[.lead]
nblade.didnotinitialize events occur when the appliance's network process failed during initialization due to an internal error (likely there is not enough system memory available to start the necessary kernel threads).
