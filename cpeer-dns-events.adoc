...
[.lead]
cpeer.dns events are presented when an attempt is made to resolve and insert a stable address that is a Domain Name System (DNS) style host name which is typically fixed by verifying system state and updating configuration.
