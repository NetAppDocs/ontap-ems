...
[.lead]
ses.psu events are presented when the power supplies that are installed in a disk shelf are not compatible with each other which is typically fixed by verifying system state and connectivity.
