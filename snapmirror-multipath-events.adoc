...
[.lead]
snapmirror.multipath events are presented when this event is issued during boot or during takeover when the filer is cannot setup the necessary infrastructure required for SnapMirror multipathing support which is typically fixed by verifying system state and connectivity.
