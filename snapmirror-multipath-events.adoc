...
[.lead]
snapmirror.multipath events are presented when the filer is cannot setup the necessary infrastructure required for SnapMirror multipathing support during boot or during takeover, which is typically fixed by verifying system state and connectivity.
