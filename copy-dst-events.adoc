...
[.lead]
copy.dst events are presented when this event is issued when a SnapMirror or aggregate copy operation attempts to update a flexible volume RAID block on the destination but is cannot do so which is typically fixed by verifying system state and connectivity.
