...
[.lead]
callhome.okm events are presented when the cluster has reached an inconsistent state with respect to the onboard key hierarchy which is typically fixed by following the documented corrective steps.
...
*Corrective Action*::
The Onboard Key Manager hierarchy is incomplete. Run the "security key-manager onboard sync" command to restore the Onboard Key Manager hierarchy.
*Syslog Message*::
