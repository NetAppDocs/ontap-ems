...
[.lead]
callhome.jpc events are presented when the Kernel Cluster Services (KCS) / Cluster Transaction Manager (CTRAN) detects that the algorithm to reselect the Join Proposal Coordinator (JPC) is failing repeatedly which may require contacting NetApp support.
