...
[.lead]
ha.takeoverimpdegraded events are presented when the partner node is down and it is currently impossible to perform a takeover because the partner mailbox disks are not available which is typically fixed by verifying system state and connectivity.
