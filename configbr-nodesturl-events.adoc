...
[.lead]
configbr.nodesturl events are presented when the cluster backup is saved on only one node and no offsite configuration backup destination URL is configured which is typically fixed by updating configuration.
