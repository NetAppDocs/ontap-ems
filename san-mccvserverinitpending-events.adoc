...
[.lead]
san.mccvserverinitpending events are presented when, in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems, SAN Vservers exceed the time allotted for initialization during a switchover or switchback operation, which may require contacting NetApp support.
