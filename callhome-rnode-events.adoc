...
[.lead]
callhome.rnode events are presented when a new set of log files has been collected from a Service Processor (SP) of one or more non-operational nodes in the cluster, which typically does not require corrective action.
