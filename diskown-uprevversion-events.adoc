...
[.lead]
diskown.uprevversion events are presented when disk ownership information has been read and found to be from a version that is higher (newer) than what the current version of Data ONTAP understands which typically does not require corrective action.
