...
[.lead]
mcc.network events are presented when a MetroCluster over IP configuration detects network congestion over the inter-cluster links (ISL) that connect sites and an action is taken to reduce further network congestion which is typically fixed by verifying system state and connectivity.
