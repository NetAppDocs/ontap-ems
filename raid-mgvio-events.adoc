...
[.lead]
raid.mgvio events are presented when after detecting an inconsistent RAID write signature on a RAID stripe, during SyncMirror verify operation, which typically does not require corrective action.
