...
[.lead]
cf.badshelfcountversion events are presented when if we are sent a negotiated failover shelf count message with a version, which we don't support which is typically fixed by following the documented corrective steps.
