...
[.lead]
nblade.vscanbadipprivaccess events are presented when the IP address of that client is not found in the list of allowed IP addresses during a client attempt to connect to the privileged ONTAP_ADMIN$ share, which is typically fixed by verifying system state and connectivity.
