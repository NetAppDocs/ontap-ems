...
[.lead]
asup.post events are presented when the AutoSupport(R) mechanism quits trying to post to the NetApp URL after having retried for the specified number of times which is typically fixed by verifying system state and connectivity.
