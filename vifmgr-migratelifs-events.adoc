...
[.lead]
vifmgr.migratelifs events occur when the network load balancing listener is not responding to probes over a node management LIF, and cluster-scoped LIFs with the same current port have been migrated elsewhere.
