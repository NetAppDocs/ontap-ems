...
[.lead]
wafl.maxdirsize events are presented when for each volume that has a maximum directory size greater than the default during boot which is typically fixed by updating configuration.
