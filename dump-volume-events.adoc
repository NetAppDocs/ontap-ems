...
[.lead]
dump.volume events are presented when this event is generated when we are trying to dump a volume which is full and we cannot write the offsetmap on to the disk and we are using memory instead of disk to do this which is typically fixed by following the documented corrective steps.
