...
[.lead]
nblade.cifsetcdmaxtxnlimit events are presented when the number of Access Control Entries (ACE) present in an MSRPC request for updating share permissions exceeds the supported maximum etcd limit of 1,022 entries which is typically fixed by following the documented corrective steps.
