...
[.lead]
snapvault.src events are presented when a SnapVault(R) source appliance receives a transfer request when the service is not enabled which is typically fixed by following the documented corrective steps.
