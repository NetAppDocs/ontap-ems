...
[.lead]
sis.c2c events are presented when this event is issued when dense scanner could not start on c2c restore volume during restore operation which is typically fixed by following the documented corrective steps.
