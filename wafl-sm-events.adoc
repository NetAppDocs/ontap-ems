...
[.lead]
wafl.sm events are presented when snapMirror(R) fails to read a block from the container file of a FlexVol(R) SnapMirror source volume which may require contacting NetApp support.
