...
[.lead]
pcnfsd.auth events are presented when the authorization for a user fails due to a corrupted password entry in /etc/passwd which is typically fixed by following the documented corrective steps.
