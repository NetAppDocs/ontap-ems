...
[.lead]
api.input events are presented when an ONTAPI(tm) API receives extra input parameters as part of the API request or when a required input parameter is missing in the API request which is typically fixed by updating configuration.
