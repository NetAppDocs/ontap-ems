...
[.lead]
sfu.fcdownloadfailed EMS events occur when a disk shelf firmware update fails to download shelf firmware to an FC or an ATA shelf successfully.
...
*Corrective Action*::
Redownload the latest disk shelf firmware from the NetApp Support Site at https://mysupport.netapp.com/site/downloads/firmware/disk-shelf-firmware and attempt to download disk shelf firmware again by using the (advanced privilege) "storage shelf firmware update" command.
*Syslog Message*::
