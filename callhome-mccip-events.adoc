...
[.lead]
callhome.mccip events are presented when automatic unplanned switchover (AUSO) in a MetroCluster(tm) IP configuration is disabled because the mediator mailbox disk is not healthy which is typically fixed by verifying system state and connectivity.
...
NOTICE
*Description*::
This message occurs when MetroCluster IP statistics are collected. TCP/IP networking statistics for diagnostic purposes are collected.
*Corrective Action*::
(None).
*Syslog Message*::
Call home for %s.
