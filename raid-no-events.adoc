...
[.lead]
raid.no events are presented when periodically to warn the user that the aggregate has no RAID protection and is not safe for storing data which is typically fixed by following the documented corrective steps.
