...
[.lead]
rdb.env events are presented when the cluster management Data Replication Module determines that I/O operations on the node's mroot filesystem are failing which may require contacting NetApp support.
