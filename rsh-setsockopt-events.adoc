...
[.lead]
rsh.setsockopt events are presented when the setsockopt option on the RemoteShell(RSH) socket fails; RSH is not available which is typically fixed by following the documented corrective steps.
