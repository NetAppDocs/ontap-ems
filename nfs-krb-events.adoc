...
[.lead]
nfs.krb events are presented when a Kerberos-enabled LIF in a Vserver shares NFS Kerberos credentials with a LIF in another Vserver which is typically fixed by following the documented corrective steps.
