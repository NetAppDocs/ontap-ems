...
[.lead]
secd.nfs events are presented when data ONTAP(R) truncates the number of groups to which a user belongs which is typically fixed by verifying system state and updating configuration.
