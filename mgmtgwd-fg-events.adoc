...
[.lead]
mgmtgwd.fg events occur when the user creates a FlexGroup on a Vserver that has NFSv3 enabled, but uses 32-bit identifiers, which could cause file ID collisions.
