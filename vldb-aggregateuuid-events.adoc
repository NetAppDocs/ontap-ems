...
[.lead]
vldb.aggregateuuid events are presented when an aggregate's Universally Unique Identifier (UUID) in the Volume Location Database does not match the aggregate's current UUID which typically does not require corrective action.
