...
[.lead]
security.cluster events are presented when a digital certificate associated with a node for cluster-network security is already expired which is typically fixed by updating configuration.
