...
[.lead]
connectx.ibretimerfailcount events are presented when infiniBand retimer programming fails due to one of the following reasons: InfiniBand device not found, GPIO read failure or retimer chip failure which is typically fixed by following the documented corrective steps.
