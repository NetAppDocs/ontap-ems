...
[.lead]
smbc.cg events occur when read of Consistency Group aggregate information entry fails for the SnapMirror(R) active sync volume which results into IO disruption.
