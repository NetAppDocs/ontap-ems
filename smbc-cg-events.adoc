...
[.lead]
smbc.cg events are presented when read of Consistency Group aggregate information entry fails for the SnapMirror(R) active sync volume which results into IO disruption which is typically fixed by following the documented corrective steps.
