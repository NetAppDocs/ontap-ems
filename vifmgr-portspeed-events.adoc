...
[.lead]
vifmgr.portspeed events are presented when a port is connected to a switch utilizing a link speed that is not qualified which is typically fixed by following the documented corrective steps.
