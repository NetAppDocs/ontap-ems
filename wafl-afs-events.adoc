...
[.lead]
wafl.afs events are presented when the deswizzle scanner fails to enable the wafl.deswizzle.afs.first feature on the aggregate because block reallocation redirection cannot be enabled on the aggregate which typically does not require corrective action.
