...
[.lead]
mgmt.cifsshare events occur when a network client attempts to set a CIFS share's discretionary access control list (DACL) to a list where any security descriptor (SID) appears in more than one access control entry (ACE).
