...
[.lead]
snapmirror.src events are presented when this event is issued when a SnapMirror is issued from a source volume which is offline, restricted or does not exist which typically does not require corrective action.
