...
[.lead]
snapmirror.src events are presented when a SnapMirror is issued from a source volume, which is offline, restricted or does not exist which typically does not require corrective action.
