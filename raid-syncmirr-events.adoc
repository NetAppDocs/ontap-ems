...
[.lead]
raid.syncmirr events are presented when in a SyncMirror(R) configuration (with SyncMirror aggressive timeout capability), when RAID cannot service earlier timed-out I/O from the other plex which may require reseating or replacing affected hardware.
