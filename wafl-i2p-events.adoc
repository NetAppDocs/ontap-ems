...
[.lead]
wafl.i2p events are presented when the ONTAP(R) kernel detects that an i2p metafile on one of the snapshot has more entries than necessary for an inode which typically does not require corrective action.
