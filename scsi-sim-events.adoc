...
[.lead]
scsi.sim events are presented when a Check Condition is the mechanism whereby a target device reports an informational condition or error status to the requesting host which is typically fixed by verifying system state and connectivity.
