...
[.lead]
callhome.snapmirror events are presented when a sync-granular consistency group (CG) transitions from in-sync status to out-of-sync status which is typically fixed by following the documented corrective steps.
