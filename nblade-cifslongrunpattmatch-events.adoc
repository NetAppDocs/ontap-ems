...
[.lead]
nblade.cifslongrunpattmatch events are presented when a CIFS query directory command that includes wildcard characters exceeds the reporting time threshold for completion which is typically fixed by verifying system state and connectivity.
