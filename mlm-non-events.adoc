...
[.lead]
mlm.non events are presented when data ONTAP(R) detects non-optimized usage of an array's target port which is typically fixed by updating configuration.
