...
[.lead]
sp.network events are presented when either a user or the system disables the network IPv6 option and the service processor (SP) is configured only with an IPv6 address which is typically fixed by following the documented corrective steps.
