...
[.lead]
nvmf.tcp events are presented when there is an internal error while creating a NVMe-over-TCP (NVMe/TCP) logical network interface (LIF) which is typically fixed by updating configuration.
