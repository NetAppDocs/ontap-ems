...
[.lead]
nblade.nocsmsession events are presented when the local node could not acquire a Connection Session Manager (CSM) session which is typically fixed by verifying system state and connectivity.
