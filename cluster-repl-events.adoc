...
[.lead]
cluster.repl events are presented when the cluster replication heartbeat to a peer cluster cannot reach the local management gateway process for a minimum of 300 seconds which typically does not require corrective action.
