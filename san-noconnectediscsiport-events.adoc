...
[.lead]
san.noconnectediscsiport events are presented when in MetroCluster(TM) configurations on clustered ONTAP(R) systems, when ONTAP cannot replicate an IP logical interface (LIF) to a candidate port in the destination cluster which is typically fixed by verifying system state and updating configuration.
