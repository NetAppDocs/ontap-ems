...
[.lead]
mgmtgwd.replace events are presented when the temporary node management LIF, which is normally deleted automatically, has not been replaced which is typically fixed by following the documented corrective steps.
