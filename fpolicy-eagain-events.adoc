...
[.lead]
fpolicy.eagain events are presented when the TCP connection to the FPolicy server is broken four times in a two minute interval due to EAGAIN error which is typically fixed by verifying system state and connectivity.
