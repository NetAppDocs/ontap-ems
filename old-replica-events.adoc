...
[.lead]
old.replica events are presented when a wafliron operation determines that a volume served as a SnapMirror(R) destination for a previous release of Data ONTAP(R) which is typically fixed by following the documented corrective steps.
