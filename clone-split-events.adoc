...
[.lead]
clone.split events are presented when the volume clone split object copy operation (which copies objects from source volume to a clone) fails because it is cannot read source objects due to unauthorized access to the object store container which is typically fixed by updating configuration.
