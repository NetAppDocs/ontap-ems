...
[.lead]
vsdr.volmovsrcfpnonfpaggr events are presented when a volume move operation is executed between FabricPool and non-FabricPool aggregates on the source Vserver of an SVM disaster recovery (SVMDR) relationship which is typically fixed by following the documented corrective steps.
