...
[.lead]
raid.aggr events are presented when the usable size of an aggregate does not expand after a corresponding increase in the size of the underlying storage which is typically fixed by verifying system state and connectivity.
