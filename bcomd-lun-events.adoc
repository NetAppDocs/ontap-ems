...
[.lead]
bcomd.lun events are presented when the SAN Kernel Agent is cannot process a LUN metadata update resulting from a Snapshot(R) copy restore operation or other LUN event which is typically fixed by following the documented corrective steps.
