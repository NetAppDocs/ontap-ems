...
[.lead]
moveagent.abort events are presented when this event is issued during volume move when cutover is aborted because the volume took too long to replicate which is typically fixed by following the documented corrective steps.
