...
[.lead]
moveagent.abort events are presented when cutover is aborted because the volume took too long to replicate during volume move, which is typically fixed by following the documented corrective steps.
