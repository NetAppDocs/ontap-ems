...
[.lead]
sla.shelf events are presented when a shelf firmware module is rebooted administratively, or when a reboot occurs normally when the system powers up, the firmware is updated, or the alternate control path (ACP) is reset which typically does not require corrective action.
