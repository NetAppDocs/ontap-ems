...
[.lead]
dump.inconsistent events are presented when the dump engine encounters a disk error in an ACL file in a Snapshot(tm) copy which may require contacting NetApp support.
