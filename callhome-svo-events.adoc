...
[.lead]
callhome.svo events are presented when a write to a SnapValidator enabled volume failed to be validated and was rejected by the server which is typically fixed by verifying system state and updating configuration.
