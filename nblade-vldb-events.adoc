...
[.lead]
nblade.vldb events are presented when a protocol requests a list of blades but one cannot be generated due to one or more references to a previous request or requests still outstanding which may require a reboot.
...
*Syslog Message*::
%s request from the network module to the VLDB times out.
*Parameters*::
*pMessage* (STRING): Message to indicate the function that triggers the error.
