...
[.lead]
cf.disk events are presented when one of the nodes in a high-availability (HA) pair reports that multiple disks in the disk inventory do not match what the partner node reported which is typically fixed by verifying system state and connectivity.
