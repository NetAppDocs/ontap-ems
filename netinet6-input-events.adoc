...
[.lead]
netinet6.input events are presented when the destination address is not ready, so the packet is discarded which typically does not require corrective action.
