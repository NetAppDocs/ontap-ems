...
[.lead]
moveagent.switchback events are presented when the volume move operation vetoes a MetroCluster(tm) switchback because the operations are mutually exclusive which is typically fixed by verifying system state and connectivity.
