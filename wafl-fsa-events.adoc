...
[.lead]
wafl.fsa events are presented when enabling file system analytics on a volume with autosize enabled fails because there is insufficient space in the volume which is typically fixed by updating configuration.
