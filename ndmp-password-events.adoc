...
[.lead]
ndmp.password events are presented when the password for the NDMP user account is still the default password on NDMP service startup, which is typically fixed by following the documented corrective steps.
