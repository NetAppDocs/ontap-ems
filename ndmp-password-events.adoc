...
[.lead]
ndmp.password events are presented when on NDMP service startup, when the password for the NDMP user account is still the default password which is typically fixed by following the documented corrective steps.
