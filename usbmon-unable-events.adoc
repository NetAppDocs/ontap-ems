...
[.lead]
usbmon.unable events are presented when the Data ONTAP(R) module that is responsible for monitoring the health of the Universal Serial Bus (USB) boot devices cannot extract health information from the monitored device which may require reseating or replacing affected hardware.
