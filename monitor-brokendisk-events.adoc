...
[.lead]
monitor.brokendisk events are presented when a node is in a degraded state because of a broken disk which is typically fixed by verifying system state and updating configuration.
