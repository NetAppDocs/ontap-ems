...
[.lead]
callhome.encl events are presented when contact to one or more disk shelf enclosure services (environmental monitoring) is lost, possibly due to storage cabling, adapter, or configuration problems which is typically fixed by verifying system state and connectivity.
