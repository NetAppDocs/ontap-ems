...
[.lead]
vifmgr.cluscheck events are presented when the named cluster logical interface (LIF) was not assigned to any network port and is now assigned to a port which typically does not require corrective action.
...
NOTICE
*Description*::
This message occurs when all packets of the ping test are now successfully received. The ping test verifies the connectivity of a cluster logical interface (LIF) to all other cluster LIFs.
*Corrective Action*::
(None).
*Syslog Message*::
