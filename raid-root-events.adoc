...
[.lead]
raid.root events are presented when the root volume is not mirrored in a MetroCluster(tm) configuration which is typically fixed by following the documented corrective steps.
