...
[.lead]
aws.dev events are presented when oNTAP(R) is running as an instance in Amazon Web Services (AWS), and a data disk failure occurs because of an I/O hang which is typically fixed by following the documented corrective steps.
