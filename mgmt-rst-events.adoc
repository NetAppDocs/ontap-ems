...
[.lead]
mgmt.rst events are presented when the SnapMirror(R) 'restore' command fails because compression is enabled on the destination volume which is typically fixed by following the documented corrective steps.
