...
[.lead]
fips.post events are presented when an unrecoverable error prevents initialization of the cryptographic subsystem into FIPS mode which may require restarting affected services or nodes.
