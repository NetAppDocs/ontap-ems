...
[.lead]
raid.mgrio events occur during a SyncMirror(R) resynchronization operation, when the system detects an inconsistent RAID write signature on a RAID stripe.
