...
[.lead]
raid.mgrio events are presented when the system detects an inconsistent RAID write signature on a RAID stripe during a SyncMirror(R) resynchronization operation, which typically does not require corrective action.
