...
[.lead]
raid.mgrio events are presented when during a SyncMirror(R) resynchronization operation, when the system detects an inconsistent RAID write signature on a RAID stripe which typically does not require corrective action.
