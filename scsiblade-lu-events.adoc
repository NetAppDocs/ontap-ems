...
[.lead]
scsiblade.lu events are presented when this node completes a group internal rebuild for an LU which typically does not require corrective action.
...
== scsiblade.lu.map.ns.unexp
*Severity*::
ERROR
*Description*::
This message occurs when the system detects an NVMe namespace mapped to an initiator group. Only LUNs should be mapped to igroups; a mapped namespace can lead to system disruption and instability. As a precaution, the namespace map is ignored.
*Corrective Action*::
Contact NetApp technical support.
*Syslog Message*::
Ignoring LUN map with Vdisk ID: %s in Volume MSID 0x%llx on storage virtual machine(SVM) %s.
*Parameters*::
*vdiskId* (STRING): Vdisk ID of the LUN.
*volumeMSID* (LONGINTHEX): Master set ID(MSID) of the volume that contains the namespace.
*vserver* (STRING): Universal unique identifier(UUID) of the storage virtual machine(SVM) that hosts the LUN.
