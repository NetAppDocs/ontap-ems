...
[.lead]
km.restore events are presented when the key manager attempts to restore a key which has an invalid length which is typically fixed by following the documented corrective steps.
