...
[.lead]
nameserv.netgrpbyhost events are presented when netgroup is configured but the netgroup byhost database is not generated during an upgrade operation which is typically fixed by verifying system state and connectivity.
