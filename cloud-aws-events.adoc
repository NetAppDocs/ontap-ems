...
[.lead]
cloud.aws events are presented when the IAM Role thread acquires Amazon Web Services (AWS) Identity and Access Management (IAM) role-based credentials that have already expired which is typically fixed by verifying system state and connectivity.
