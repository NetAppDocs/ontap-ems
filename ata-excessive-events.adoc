...
[.lead]
ata.excessive events are presented when an I/O request to the ATA driver in ONTAP(R) Select takes longer than five seconds which is typically fixed by verifying system state and connectivity.
