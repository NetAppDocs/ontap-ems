...
[.lead]
ipmultipath.init events are presented when during IP Multipath session initialization, when an INIT packet with an unknown checksum type is received which is typically fixed by following the documented corrective steps.
