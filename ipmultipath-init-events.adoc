...
[.lead]
ipmultipath.init events are presented when an INIT packet with an unknown checksum type is received during IP Multipath session initialization, which is typically fixed by following the documented corrective steps.
