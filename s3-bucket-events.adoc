...
[.lead]
s3.bucket events are presented when the access key is invalid and it has been used to access the S3 bucket/object which is typically fixed by verifying system state and connectivity.
