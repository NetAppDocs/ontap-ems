...
[.lead]
rlm.heartbeat events are presented when the appliance detects resumption of Remote LAN Module (RLM) heartbeat notifications indicating that the RLM is now available, which typically does not require corrective action.
