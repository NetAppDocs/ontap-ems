...
[.lead]
cifs.does events are presented when a Vserver or Cluster administrator tries to initialize Snapmirror on Async VserverDR ID Discard setup without creating a CIFS server at destination which is typically fixed by following the documented corrective steps.
