...
[.lead]
raid.warn events are presented when there are no suitable spare disks left on the node to use for a potential future coredump operation which is typically fixed by following the documented corrective steps.
