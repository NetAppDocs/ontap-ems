...
[.lead]
sfo.aggr events are presented when a storage failover aggregate is relocated from one node to another during a nondisruptive controller upgrade which typically does not require corrective action.
