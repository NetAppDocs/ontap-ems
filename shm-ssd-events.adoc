...
[.lead]
shm.ssd EMS events occur when ONTAP(R) detects that a solid-state drive (SSD) has an average block latency during a five-minute timeframe that is greater than expected.
