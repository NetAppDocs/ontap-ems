...
[.lead]
shm.ssd events are presented when oNTAP(R) detects that a solid-state drive (SSD) has an average block latency during a five-minute timeframe that is greater than expected which typically does not require corrective action.
