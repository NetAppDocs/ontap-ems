...
[.lead]
wafl.qtree events are presented when a qtree ID derived from a WAFL(R) buffer does not match the given tree ID in a snapshot which is typically fixed by updating configuration.
