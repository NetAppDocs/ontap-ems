...
[.lead]
wafl.cbin events are presented when cloud mirror configuration changes during the WAFLiron operation on a FabricPool aggregate with unified format enabled which is typically fixed by following the documented corrective steps.
