...
[.lead]
callhome.clus events are presented when the cluster network has experienced an issue that could potentially affect connectivity, reliability, redundancy, or performance, as observed from the local node which is typically fixed by verifying system state and updating configuration.
