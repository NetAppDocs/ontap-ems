...
[.lead]
san.nofcportwithadaptertype events are presented when in MetroCluster(tm) configurations on clustered Data ONTAP(R) systems, when Data ONTAP cannot replicate an FC/FC-NVMe logical interface (LIF) to a candidate port in the destination cluster which is typically fixed by verifying system state and connectivity.
