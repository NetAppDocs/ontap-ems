...
[.lead]
scsitarget.hwpfct events are presented when an unexpected event or illegal condition is detected by the FC target driver and the contents of the FC target's memory is saved which may require contacting NetApp support.
