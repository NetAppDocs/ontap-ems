...
[.lead]
clam.enable events are presented when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) is enabled to operate on this node which is typically fixed by following the documented corrective steps.
