...
[.lead]
snaplock.ss events occur when if an attempt is made to lock a snapshot on a SnapLock volume that has previously failed to find or load its SnapLock snapshot state.
