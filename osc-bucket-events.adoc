...
[.lead]
osc.bucket events are presented when the object-store bucket or container is not found while trying to access an object which is typically fixed by verifying system state and connectivity.
