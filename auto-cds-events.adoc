...
[.lead]
auto.cds events are presented when volume efficiency inactive data compression scan disablement failed on the existing compatible volumes during upgrade to Data ONTAP(R) 9.11.1 or later, which is typically fixed by following the documented corrective steps.
