...
[.lead]
auto.cds events are presented when during upgrade to Data ONTAP(R) 9.11.1 or later, when volume efficiency inactive data compression scan disablement failed on the existing compatible volumes which is typically fixed by following the documented corrective steps.
