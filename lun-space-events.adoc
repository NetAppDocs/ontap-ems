...
[.lead]
lun.space events are presented when the space reservation settings for the named volume are not honored in its containing aggregate which is typically fixed by updating configuration.
