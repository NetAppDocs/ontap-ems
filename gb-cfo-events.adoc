...
[.lead]
gb.cfo events are presented when a high-availability (HA) controller failover (CFO) giveback is canceled due to RAID activity on a partner aggregate which may require a reboot.
