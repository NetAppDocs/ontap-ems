...
[.lead]
raid.syncmirror events are presented when, in a SyncMirror(R) configuration (with SyncMirror aggressive timeout capability), a disk I/O times out with an error, which is typically fixed by following the documented corrective steps.
