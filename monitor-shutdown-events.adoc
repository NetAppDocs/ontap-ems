...
[.lead]
monitor.shutdown events are presented when an automatic shutdown sequence is initiated due to a degraded RAID group that cannot be reconstructed because there are insufficient appropriate spare disks which is typically fixed by following the documented corrective steps.
