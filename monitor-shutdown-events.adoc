...
[.lead]
monitor.shutdown events occur when an automatic shutdown sequence is initiated due to a degraded RAID group that cannot be reconstructed because there are insufficient appropriate spare disks.
