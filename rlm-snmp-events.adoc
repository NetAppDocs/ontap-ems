...
[.lead]
rlm.snmp events are presented when each time a system boots, if the advanced privilege level in Data ONTAP(R) was used to disable the SNMP Trap feature of the Remote LAN Module (RLM) which is typically fixed by updating configuration.
