...
[.lead]
nblade.cifsnbnameconflict events are presented when the NetBIOS Name Service receives a negative response from a remote machine for a name registration request which is typically fixed by following the documented corrective steps.
