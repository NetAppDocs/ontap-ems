...
[.lead]
moveagent.swapvldb events are presented when a 'volume move' operation finishes moving the volume to the destination but fails to update the volume location database (VLDB) which is typically fixed by verifying system state and connectivity.
