...
[.lead]
sblade.duplicate events are presented when a duplicate IP address is detected on more than one iSCSI interface which is typically fixed by following the documented corrective steps.
