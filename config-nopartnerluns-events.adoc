...
[.lead]
config.nopartnerluns events are presented when in a High-Availablity (HA) configuration with array LUNS when there is no attachment to both local and partner LUNs which is typically fixed by verifying system state and connectivity.
