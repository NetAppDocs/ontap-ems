...
[.lead]
callhome.chassis events are presented when the chassis temperature is too warm and might cause an environmental problem, or when there is faulty hardware in the system, such as a fan failure or a malfunctioning temperature sensor which may require reseating or replacing affected hardware.
