...
[.lead]
smtape.bkp events are presented when an SMTape backup operation is aborted because a write to tape device fails which is typically fixed by verifying system state and connectivity.
