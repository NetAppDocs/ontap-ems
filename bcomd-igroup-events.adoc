...
[.lead]
bcomd.igroup events are presented when an initiator group marked with the delete-on-unmap flag is not automatically deleted when it is no longer part of a LUN mapping relationship which is typically fixed by following the documented corrective steps.
