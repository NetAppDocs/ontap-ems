...
[.lead]
lun.transition events are presented when the transition of a 7-Mode LUN to clustered Data ONTAP(R) fails because the i2p database for the destination volume is not fully built which is typically fixed by following the documented corrective steps.
