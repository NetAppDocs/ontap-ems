...
[.lead]
dump.fail events are presented when while dump is trying to restart and there is no space left in the snap reserve, which is typically fixed by updating configuration.
