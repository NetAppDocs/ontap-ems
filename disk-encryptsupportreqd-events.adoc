...
[.lead]
disk.encryptsupportreqd events are presented when oNTAP(R) software detects one or more nonencrypting drives or non-FIPS-certified self-encrypting drives (SEDs), but the environment variable 'bootarg.storageencryption.support' is set to "true" to indicate that all drives must be FIPS-certified which is typically fixed by following the documented corrective steps.
