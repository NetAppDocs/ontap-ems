...
[.lead]
token.inactive events are presented when a reference is deleted on a token that has existed for longer than the timeout window which typically does not require corrective action.
