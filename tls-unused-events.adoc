...
[.lead]
tls.unused events are presented when a cipher suite that is enabled in the configuration has not been used in any TLS handshakes for a user-specified duration of time which is typically fixed by verifying system state and updating configuration.
