...
[.lead]
sp.heartbeat events are presented when the appliance detects resumption of Service Processor (SP) heartbeat notifications indicating that the Service Processor is now available, which typically does not require corrective action.
