...
[.lead]
raid.monitor events are presented when the hourly monitor indicates that the current aggregate or volume number exceeded the maximum number of supported volumes on the host which is typically fixed by verifying system state and connectivity.
