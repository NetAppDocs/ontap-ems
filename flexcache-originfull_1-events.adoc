...
[.lead]
flexcache.originfull_1 events are presented when a FlexCache(R) volume cannot retrieve data from its origin volume because origin volume is full which is typically fixed by following the documented corrective steps.
