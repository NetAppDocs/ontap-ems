...
[.lead]
moveagent.detected events are presented when a volume move operation is interrupted by a disaster after the critical cutover phase has started but before the source volume is deleted which is typically fixed by verifying system state and connectivity.
