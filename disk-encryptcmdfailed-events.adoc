...
[.lead]
disk.encryptcmdfailed events are presented when the indicated "storage encryption disk" command fails on an encrypting disk that supports storage encryption methods which is typically fixed by verifying system state and connectivity.
