...
[.lead]
vifmgr.ipspace events are presented when the system is over-provisioned with IPspaces, exceeding the maximum number allowed.The over-provisioned IPspaces might not be configured successfully in the stack which is typically fixed by following the documented corrective steps.
