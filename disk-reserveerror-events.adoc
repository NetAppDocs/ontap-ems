...
[.lead]
disk.reserveerror events are presented when a SCSI reservation is requested for a disk or LUN that is no longer seen which is typically fixed by verifying system state and connectivity.
