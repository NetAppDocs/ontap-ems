...
[.lead]
lmgr.sf EMS events occur when the lock manager non-disruption subsystem for CIFS continuously available (CA) shares notifies the high-availability (HA) module that transition into the up state is allowed.
