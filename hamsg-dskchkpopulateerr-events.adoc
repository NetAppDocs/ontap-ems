...
[.lead]
hamsg.dskchkpopulateerr events are presented when a disk inventory check cannot be sent to the local node's high-availability (HA) partner because the disk check request could not be correctly constructed which is typically fixed by verifying system state and connectivity.
