...
[.lead]
secd.strong events are presented when the external LDAP server configured for a Vserver is enforcing stronger authentication than what the ONTAP(R) LDAP client is configured for which is typically fixed by updating configuration.
