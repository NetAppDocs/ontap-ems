...
[.lead]
nblade.tlsconfigerror events are presented when the client is trying to use NFS over TLS (Transport Layer Security), but it is disabled on the NFS server side which is typically fixed by following the documented corrective steps.
