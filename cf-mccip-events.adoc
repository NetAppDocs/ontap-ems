...
[.lead]
cf.mccip events are presented when automatic switchover in a MetroCluster(tm) IP configuration is disabled because the mediator mailbox disk is not healthy which is typically fixed by verifying system state and connectivity.
