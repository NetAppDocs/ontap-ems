...
[.lead]
sfu.statuscheckfailure events are presented when the (advanced privilege) "storage shelf firmware update" command encounters a failure while attempting to read the status of the firmware update in progress which is typically fixed by updating configuration.
