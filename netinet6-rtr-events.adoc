...
[.lead]
netinet6.rtr events are presented when the Maximum Transmission Unit (MTU) size of the Router Advertisement (RA) message, sent from the specified source IPv6 address, is higher than the upper bound of the Link MTU which typically does not require corrective action.
