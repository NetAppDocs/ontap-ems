...
[.lead]
cifs.shadowcopy events are presented when a shadow copy set is automatically aborted when no activity has been detected for a certain period of time which is typically fixed by verifying system state and connectivity.
