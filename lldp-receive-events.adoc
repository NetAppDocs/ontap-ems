...
[.lead]
lldp.receive events are presented when the LLDP (Link Layer Discovery Protocol) daemon running on the storage controller receives more unique neighbor advertisements than it can cache which is typically fixed by following the documented corrective steps.
