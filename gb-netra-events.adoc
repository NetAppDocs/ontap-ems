...
[.lead]
gb.netra events are presented when during the migration of an aggregate as a part of storage failover (SFO) giveback, when the destination node cannot reach the object stores which may require a reboot.
