...
[.lead]
gb.netra events are presented when the destination node cannot reach the object stores during the migration of an aggregate as a part of storage failover (SFO) giveback, which may require a reboot.
