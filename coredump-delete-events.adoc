...
[.lead]
coredump.delete events are presented when a requested coredump delete cannot occur because an aggregate relocation or a takeover is in progress which is typically fixed by following the documented corrective steps.
