...
[.lead]
vsdr.vol events are presented when a Vserver DR subsystem does not enable inline deduplication on the destination volume which is typically fixed by following the documented corrective steps.
