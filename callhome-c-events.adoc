...
[.lead]
callhome.c events are presented when there is an unexpected decrease in a chassis fan's spin rate, possibly due to a slight decrease in revolutions or to a failed chassis fan which is typically fixed by verifying system state and connectivity.
